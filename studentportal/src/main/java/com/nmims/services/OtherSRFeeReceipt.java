...
@Service("otherSrreceipt")
public class OtherSRFeeReceipt implements FeeReceiptInterface {
	...
	public void createSrFeeReceipt(String FEE_RECEIPT_PATH, ServiceRequestStudentPortal sr, StudentStudentPortalBean student, String courierAmount, ArrayList<ServiceRequestStudentPortal> marksheetDetailAndAmountToBePaidList) throws Exception {
		try {
			...
			String examPeriod =sr.getServiceRequestType().toUpperCase()+" SERVICE REQUEST FEE RECEIPT";
			if((sr.getYear()!=null) && (sr.getMonth()!=null)) {
				examPeriod=examPeriod+" FOR "+sr.getMonth().toUpperCase()+"-"+sr.getYear();
			}
			Chunk examPeriodChunk = new Chunk(examPeriod, font2);
