...
<%@ include file="header.jsp"%>
    ...
    <section class="content-container login">
        <div class="container-fluid customTheme">
					...
					<table class="table table-striped panel-body">
						...
						<c:forEach var="announcement" items="${announcements}" varStatus="status">
					        <tr>
       					  ...
       					  <td ><center><a href="javascript:void(0)" data-id="${ announcement.id }" class="commonLinkbtn">Common Announcement For ${ announcement.count } programs</center></a></td>
					            ...
					            <td> 
						            <c:url value="editAnnouncement" var="editurl">
									  <c:param name="id" value="${announcement.id}" />
									</c:url>
									<c:url value="deleteAnnouncement" var="deleteurl">
									  <c:param name="id" value="${announcement.id}" />
									</c:url>
									<c:url value="viewAnnouncementDetails" var="detailsUrl">
									  <c:param name="id" value="${announcement.id}" />
									</c:url>
									<a class="glyphicon glyphicon-info-sign" href="${detailsUrl}" title="Details"></a>
									...
									<a class="glyphicon glyphicon-trash" href="${deleteurl}" title="Delete" onclick="return confirm('Are you sure you want to delete this Announcement?')"></a>
									<%} %>
					            </td>
					        </tr>   
					    </c:forEach>
			...
			<c:url var="firstUrl" value="getAllAnnouncementsPage?pageNo=1" />
			<c:url var="lastUrl"
			value="getAllAnnouncementsPage?pageNo=${page.totalPages}" />
			<c:url var="prevUrl"
			value="getAllAnnouncementsPage?pageNo=${page.currentIndex - 1}" />
			<c:url var="nextUrl"
			value="getAllAnnouncementsPage?pageNo=${page.currentIndex + 1}" />
