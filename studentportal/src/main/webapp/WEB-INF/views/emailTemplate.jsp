			...
			<div class="panel-body" style="margin-top: 40px;">
					...
					<div class="form-group col-md-12" style="margin-top: 10px;">
						<select class="form-control" id='module' style="text-transform: capitalize;" onchange="moduleChangeHandler()">
							...
							<c:forEach var="module" items="${ modules }">
								<option value="${ module.id }" style="text-transform: capitalize;">${ module.name } - ${ module.topic }</option>
							</c:forEach>
						</select>
					</div>
				</div>
				...
				<div class="row">
					<div class="form-group col-md-12">
						...
						<button class='btn btn-primary' style="margin-top:10px; border-radius: 4px;" id='updateDetials' onclick="updateMessage()" disabled="disabled">
						Update</button>
					</div>
				</div>
			...
			$.ajax({
				...
				success : function(data) {
					...
					for(let i=0; i<data.length; i++){
						...
						$('#fromEmailId').val(data[i].fromEmailId);
						$('#subject').val(data[i].subject);
						CKEDITOR.instances.body.setData( data[i].body, function() {
							this.checkDirty();  // true
						});
