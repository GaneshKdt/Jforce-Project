					...
					<div class="panel panel-danger">
						...
						<div class="panel-body">${announcement.descriptionForDisplay}
							...
							<c:if test="${not empty  announcement.attachmentFile1Name}">
								<a target="_blank"
									href="<spring:eval expression="@propertyConfigurer.getProperty('CONTENT_PREVIEW_PATH')" />${announcement.attachment1}">${announcement.attachmentFile1Name}</a>
							</c:if>
							<br />

							<c:if test="${not empty  announcement.attachmentFile2Name}">
								<a target="_blank"
									href="<spring:eval expression="@propertyConfigurer.getProperty('CONTENT_PREVIEW_PATH')" />${announcement.attachment2}">${announcement.attachmentFile2Name}</a>
							</c:if>
							<br />

							<c:if test="${not empty  announcement.attachmentFile3Name}">
								<a target="_blank"
									href="<spring:eval expression="@propertyConfigurer.getProperty('CONTENT_PREVIEW_PATH')" />${announcement.attachment3}">${announcement.attachmentFile3Name}</a>
							</c:if>
							<br />
