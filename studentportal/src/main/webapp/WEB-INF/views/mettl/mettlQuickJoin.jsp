							...
							<jsp:useBean id="now" class="java.util.Date"/>
								...
								<div class="panel-content-wrapper">
									...
									<div class="row">
										<div class="col-md-12">	
											<c:choose>
												<c:when test="${ accessStartDateTime < now && accessEndDateTime > now }">
												...
												<c:otherwise>
													  <button type="button" onClick="startAssessment();" class="btn btn-primary" style="float: right;">
														Click Here To Start The Final Exam (Link will be Active at ${mettlExamUpcomingBean.accessStartDateTime })
													</button>
												</c:otherwise>
											</c:choose>
										</div>
								...
								<div class="row">
					<a href="/studentportal/logout" class="btn btn-primary"
...
<script>
		function startAssessment() {
			...
			if (current_time.getTime() <= exam_end_time.getTime() 
                && current_time.getTime() >= exam_start_time.getTime()) { 
					window.open("${joinLink}", "_blank");
				} else{
					  alert("Exam will start at "  + "${mettlExamUpcomingBean.accessStartDateTime }");		
				}
		}
