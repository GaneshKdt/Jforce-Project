					...
					<div class="sz-content">
						...
						<div class="panel-content-wrapper">
							...
							<p>Dear Student, You have chosen below Service Request.
								Please fill in required information and Proceed.</p>
									...
									<div class="row">
										<div class="col-md-4 column">
											...
											<div class="form-group">
												...
												<form:select path="purpose" id="reason-select" 
 													required="true" class="form-control"
 													onChange="reasonChange(this);"> 
 													<form:option value="" disabled="true" selected="true" label="Select a Reason"/> 
 													<form:option value="Record Purpose" label="Record Purpose"/> 
 													<form:option value="Official Purpose" label="Official Purpose"/> 
 													<form:option value="Scholarship Purpose" label="Scholarship Purpose"/> 
 													<form:option value="Loan Purpose" label="Loan Purpose"/> 
 													<form:option value="VISA Purpose" label="VISA Purpose"/> 
 													<form:option value="Other" label="Other"/> 
 												</form:select> 
											</div>

											<div class="form-group" id="otherReasonDiv" style="display: none">
												<label for="otherReason">Reason for Bonafide</label>
												<textarea name="additionalInfo1" class="form-control" 
													required="true" id="otherReason" cols="50" rows="5"></textarea>
											</div>
											...
											<div class="form-group">
												<label>(Certificate Shipping Charges INR. 100/-)</label>
												</br>
											...
											<div class="form-group">
												<label class="control-label" for="submit"> <a
													...
													name="submit" class="btn btn-large btn-primary " style="display: block;">Request
													For Certificate</a> <a id="backToSR"
													name="BacktoNewServiceRequest" class="btn btn-danger">Back
													to New Service Request</a></label>
												  ...
												  <div class="modal-dialog role="document"">
												    <div class="modal-content">
												      <div class="modal-header" style="text-align: center;">
												        <h4 class="modal-title" id="myModalLabel">Are you sure you want to raise Request</h4>
												      </div>
												      <div class="modal-footer" style="padding: 0.5rem;  display: flex; justify-content: space-between;">
												      <button type="button" class=" btn btn-secondary" data-dismiss="modal" id="modal-btn-cancel" style="">Cancel</button>
												       <button type="button" class="btn btn-primary" id="modal-btn-ok" value="true">Ok</button>
												    </div>
												  </div>
										      </div>
											</div>
		...
		$("#saveBonafideRequest").click(function(){
			...
			if(reason === '') {
				...
				if(!isEmptyData(reason)){
					if(reason === "Other") {
						let reasonSpecified = document.getElementById("otherReason").value.trim();

					  	if(reasonSpecified.length <= 5 && reasonSpecified.length >= 1) {
							alert("Specified reason should be 5 character or more");
							return false;   
						} 
					  	if(reasonSpecified.length == 0 ) {
							alert("Please specify your reason");
							return false;   
						} 
						 if(reasonSpecified.length >= 450) {
							alert("Specified reason cannot be more than 450 character");
							return false;   
						}
						var ok;
						 $("#mi-modal").modal('show');
...
// 		$('#addressConfirmation').prop('checked', false);
		...
		document.getElementById("chargesSpan").innerHTML = totalCost;
...
// 		}
	...
	function reasonChange(e) {
		...
		let otherReasonDiv = document.getElementById("otherReasonDiv");
