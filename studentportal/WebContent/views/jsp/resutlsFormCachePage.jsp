...
<% 
	List<String> list = Arrays.asList("ACBM", "ADSCM", "CBM", "CCC", "CDM", "CPBM");
	request.setAttribute("certificateCoursesList", list);
%>
<body>
	...
	<nav class="navbar navbar-default">
		<div class="container-fluid">
			...
			<ul class="nav navbar-nav navbar-right">
				...
				<ul class="nav navbar-nav navbar-right">
				<li class="active"><a class="btn btn-danger navbar-btn" onClick="goToPortal()">Go To Portal</a></li>
			</ul>
		</div>
	</nav>
			...
			<c:choose>
				<c:when
											...
											<tr>
												...
												<c:choose>
													...
													<c:otherwise>
														...
														<td><c:out value="${not empty recent_mark.part4marks  ? recent_mark.part4marks: ''}" /></td>
													</c:otherwise>
												</c:choose>
					...
					<div class="sz-content" id = "assignmentResult">
						<c:if test="${ studentsDataInRedisBean.resultsData.assignmentMarks.size() > 0 }">
							<h2 style="margin-left: 10px;" class="red text-capitalize"> 
							    <span>Assignment Marks records for ${ studentsDataInRedisBean.resultsData.mostRecentResultPeriod[0] } Exam </span> 
							</h2>
							<div class="clearfix"></div> 
							    <c:choose>
							        <c:when test = "${'Diageo' == studentsDataInRedisBean.resultsData.studentDetails[0].consumerType}"> 
							    ...
							    <div class="table-responsive">
							        ...
							        <tbody>
							            <c:forEach var="post" items="${ studentsDataInRedisBean.resultsData.assignmentMarks }" varStatus="i">
							                <tr> 
							                    <td> ${ (i.index + 1) } </td> 
							                    <td nowrap="nowrap" style="text-align:left;"> ${ post.subject } </td>
							                    <td> ${  post.sem } </td>
							                    <td> ${  post.assignmentscore } </td>
							                    <td> ${ post.reason != null ? post.reason : "" } </td>
							                </tr>
							            </c:forEach>
							        </tbody>
							    </table>
							...
							<div class="notesWrapper">
							    ...
							    <ul>
							        <c:choose>
							        	<c:when test="${ 'ACBM' == studentsDataInRedisBean.resultsData.studentDetails[0].program }">
							        ...
							        <c:choose>
							        	<c:when test="${ 
							        		'Online' == studentsDataInRedisBean.resultsData.studentDetails[0].examMode 
							        		&& !('JUL2017' == studentsDataInRedisBean.resultsData.studentDetails[0].prgmStructApplicable 
							        			|| fn:contains(certificateCoursesList, studentsDataInRedisBean.resultsData.studentDetails[0].program) 
							        		)
						        		}">
							            	<li>Last date to apply for Jun 2023, Assignment Revaluation is <b>27th July 2023 23:59 p.m (IST)</b> No request for assignment revaluation will be  accepted after closure of revaluation window for reasons whatsoever.</li>
							        	</c:when>
							        	<c:otherwise>
							            	<li>Last date to apply for Jun 2023, Assignment Revaluation is <b>27th July 2023 23:59 p.m (IST)</b> No request for assignment revaluation will be  accepted after closure of revaluation window for reasons whatsoever.</li>
							        	</c:otherwise>
														...
														<c:forEach var="passfail"
															...
															<tr>
																...
																<c:choose>
																	...
																	<c:when test="${'N' == passfail.isPass && 'Copy Case' == passfail.remarks }">
																		<td style="text-align: center; color: red"><b>Copy Case</b></td>
																	</c:when>
							...
							<div class="row">
								<div class="col-md-4">
									<div class="signatureLeft">
										<div>
											<img
												src="https://staticfilesexam.s3.ap-south-1.amazonaws.com/resources_2015/images/signature.jpg"
								...
								<div class="col-md-4">
									...
									<c:choose>
										...
										<c:when
											test="${'Certificate' == studentsDataInRedisBean.resultsData.studentDetails[0].programType || (fn:contains(certificateCoursesList, studentsDataInRedisBean.resultsData.studentDetails[0].program))  || 'Jul2017' == studentsDataInRedisBean.resultsData.studentDetails[0].prgmStructApplicable}">
											<p>Pass Marks:40 % on the aggregate of marks obtained in the internal assignment and Term End Examination taken together.</p>
											<p>Individual cut-off of individual Passing Criteria will be 40% for each subject.</p>
									...
									<c:if
										test='${studentsDataInRedisBean.resultsData.studentDetails[0].prgmStructApplicable  ne "Jul2017" && !(fn:contains(certificateCoursesList, studentsDataInRedisBean.resultsData.studentDetails[0].program)) }'>
										<p>
											Students who wish to apply for revaluation of the descriptive
											answers may apply for the same on or before <b> 27th July 2023 23:59 p.m </b> using Service Request link
