...
<%
	...
	ArrayList<Integer> currentSemPSSId = (ArrayList<Integer>)session.getAttribute("currentSemPSSId_studentportal");
	String pssId = StringUtils.join(currentSemPSSId, ",");
%>
	...
	<tbody>
		...
		<tr style="display: table-row;">
			...
			%><td class="bookmark video"
				data-bs-selected="yes" id="<%=video.getId()%>"><i class="fa-solid fa-bookmark"></i></td>

			<%
				} else {
			%><td class="bookmark video" data-bs-selected="no" id="<%=video.getId()%>"><i class="fa-regular fa-bookmark"  onClick = "<%= hideShow %>"></i></td>
			<%} %>

			<% 
				if(isLead == "true"){
					videoLink = "/acads/student/videosHomeNew?pageNo=1&academicCycle=All&pssId="+video.getProgramSemSubjectId();
				}else{
					videoLink = "/acads/student/watchVideos?id="+video.getId()+"&pssId="+video.getProgramSemSubjectId();
				}
			%>
			<td>			
		...
		<c:forEach var="contentFile" items="${contentList}" varStatus="status">
			<tr style="display: table-row;">
				...
				<c:if test='${contentFile.bookmarked eq "Y"}'>
					<td class="bookmark content" data-bs-selected="yes" id="${contentFile.id}"><i class="fa-solid fa-bookmark"></i></td>
				</c:if>
				<c:if test='${contentFile.bookmarked eq "N" or empty contentFile.bookmarked }'>
					<td class="bookmark content" data-bs-selected="no" id="${contentFile.id}"><i class="fa-regular fa-bookmark" onClick = "<%= hideShow %>"></i></td>
				</c:if>
				
				<td>
					<c:choose>
						<c:when test="${ isLoginAsLead eq true}">
							<a href="#" onClick="alert('Please enroll for the complete course to view the document.');">View</a>
						</c:when>
						<c:otherwise>
							...
							<c:if
								test="${fn:endsWith(contentFile.previewPath, '.pdf') || fn:endsWith(contentFile.previewPath, '.PDF')
																	|| fn:endsWith(contentFile.previewPath, '.Pdf')}">
								<c:url value="acads/student/previewContent" var="previewContentLink">
									<c:param name="previewPath" value="${contentFile.previewPath}" />
									<c:param name="name" value="${contentFile.name}" />
									<c:param name="contentFileId" value="${contentFile.id}"/>
									<c:param name="subjectName" value="${contentFile.subject}"/>
									<c:param name="pssId" value="<%= pssId %>"/>
									<c:param name="currentAcadsMonth" value="${contentFile.month}"/>
									<c:param name="currentAcadsYear" value="${contentFile.year}"/>
