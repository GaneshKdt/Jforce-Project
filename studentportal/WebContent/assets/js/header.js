...
var utterance = new SpeechSynthesisUtterance();	
		...
		//On FocusIn words will be Spoken
	  	   document.addEventListener('focusin', function(event) {			  	    
	  		 if (!event.target.classList.contains('modal') && !event.target.classList.contains('studentAnnouncementCounter')) {
	                utterance.text = document.activeElement.textContent;
	                window.speechSynthesis.speak(utterance);
	               }
	        
				if (event.target.tagName === 'INPUT' && event.target.labels.length > 0) {
					if(event.target.value){
						utterance.text = event.target.labels[0].textContent + " is " + event.target.value;
					}else{
					 	utterance.text = event.target.labels[0].textContent 
					}
    					    	        	...
    					    	        	if(responseList.length>3){
    					    	        		...
    					    	        		optionalList+='<div class="row mt-1">'
				    	        						 ...
				    	        						 for (var i = 0; i <3; i++) {
				    						    	          ...
				    						    	          optionalList += '<li class="list-group-item text-wrap" id="' + responseBean.id + '"  onClick="openPdfPage(\'' + responseBean.previewPath + '\', \'' + responseBean.name + '\');" value="'+responseBean.id+'" onclick=""  >'+ responseBean.pdfContent +'</li>';
		    					    	            ...
		    					    	            optionalList +='<div class="mt-1"><i class="fa-solid fa-clone fa-xl"></i>&nbsp;&nbsp;'
		    			    	        				...
		    			    	        				for (var i = 0; i <responseList.length; i++) {
				    						    	          ...
				    						    	          optionalList += '<li class="list-group-item text-wrap" id="' + responseBean.id + '" onClick="openPdfPage(\'' + responseBean.previewPath + '\', \'' + responseBean.name + '\');"   value="'+responseBean.pdfContent+'"  >'+ responseBean.pdfContent +'</li>';
				    						    	        }
    					    	        		}	
    					    	        ...
    					    	        if ((key==="ppt") && keywordSeparatedLists[key].length>0){
    					    	        	...
    					    	        	optionalList+='<div class="row mt-3">'
    			    						    	        ...
    			    						    	        for (var i = 0; i <3; i++) {
    			    						    	          var responseBean = responseList[i];
    			    						    	          // Perform further actions with the response bean
    			    						    	          optionalList += '<li class="list-group-item text-wrap" id="' + responseBean.id + '" value="'+responseBean.pdfContent+'"  >' + responseBean.pdfContent+'</li>';
    			    						    	        }
    					    	        	}else{
    					    	        ...
    					    	        if (key==="video"  && keywordSeparatedLists[key].length>0){
    					    	        	...
    					    	        	optionalList+='<div class="row mt-3">'
	    						    	        ...
	    						    	        for (var i = 0; i <3; i++) {
	    						    	          var responseBean = responseList[i];
	    						    	          // Perform further actions with the response bean
	    						    	          optionalList += '<li class="list-group-item text-wrap" id="' + responseBean.id + '" value="'+responseBean.transcriptContent+'" onClick="openVideoPage(\'' + responseBean.videoContentId + '\', \'' + responseBean.programSemSubjectId + '\');">' + responseBean.transcriptContent+'</li>';
	    						    	        }
    					    	        	}else{
	    					    	        optionalList +='<div class="mt-2"><i class="fa-sharp fa-regular fa-circle-play fs-4"></i>&nbsp;&nbsp;'
		    	        						 ...
		    	        						 for (var i = 0; i <responseList.length; i++) {
		    						    	          var responseBean = responseList[i];
		    						    	          // Perform further actions with the response bean
		    						    	          optionalList += '<li class="list-group-item text-wrap" id="' + responseBean.id + '"  onClick="openVideoPage(\'' + responseBean.videoContentId + '\', \'' + responseBean.programSemSubjectId + '\');"   value="'+responseBean.transcriptContent+'"  >' + responseBean.transcriptContent+'</li>';
		    						    	        }
    					    	        	} 
    					    	        ...
    					    	        if (key==="qna"  && keywordSeparatedLists[key].length>0){
    					    	        	...
    					    	        	optionalList+='<div class="row mt-3">'
		    						    	        ...
		    						    	        for (var i = 0; i <3; i++) {
		    						    	          var responseBean = responseList[i];
		    						    	          // Perform further actions with the response bean
		    						    	          optionalList += '<li class="list-group-item text-wrap" id="' + responseBean.id + '" value="'+responseBean.query+'"  >' + responseBean.query+'</li>';
		    						    	        }
    					    	        	}else{
	    					    	        optionalList +='<div class="mt-3"> <i class="far fa-comments"></i>&nbsp; &nbsp;'
