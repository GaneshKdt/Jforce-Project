...
<!-- 						added batch--> 
					...
					<div class="row">
						<div class="col-md-5">
                        	<div class="form-group">        
                            	<div id="dateSelectContainer" class="form-group" style="display:none">        
                                	<form:label path="startDate" for="startDate">Select Date</form:label>
                                    <form:select id="startDate" path="startDate" type="text" placeholder="startDate" class="form-control" itemValue="${formBean.startDate}">
                                   		<form:option value="${formBean.startDate}">${formBean.startDate}</form:option>
                                    </form:select>
								</div>
                         	</div>
                     	</div>
						...
						<div class="col-md-12">
							<div class="form-group">
								<label > Innovative Pedagogy Used : </label>
							  	<textarea name="pedagogyUsed" class="editor">NA</textarea>
							</div>
						</div>
						...
						<div class="col-md-12">
							<div class="form-group">
								<label > Case Study Name : </label>
							  	<textarea name="casestudyName" class="editor"/>NA</textarea>
							</div>
						</div>
						...
						<div class="col-md-12">
							<div class="form-group">
								<label > Pedagogical Tool : </label>
							  	<textarea name="pedagogicalTool" class="editor"/>NA</textarea>
							</div>
						</div>
					...
					<div class="">
						<div class="col-md-12">
							<div class="form-group">
								...
								<textarea name="teachingMethod" class="editor"/>NA</textarea>
							</div>
						</div>
					...
					<div class="">
						<div class="col-md-12">
							<div class="form-group">
								...
								<textarea name="casestudySource" class="editor"/>NA</textarea>
							</div>
						</div>
							...
							<tbody>
								 <c:forEach var="sessionPlan" items="${sessionPlanList}"   varStatus="status">
					        		 <tr>
										...
										<td>
											<a href="/acads/admin/editSessionPlan?id=${sessionPlan.id }" title="Edit SessionPlan" class="">
												<b>
										...
										<td>
											<a href="/acads/admin/deleteSessionPlan?id=${sessionPlan.id }" title="Delete sessionPlan" class="">
												<b>
...
<!-- Froala WYSIWYG editor JS Link START -->
...
<!-- Froala WYSIWYG editor JS Link END -->
...
function resetApplicableTypeAndSubject(){
	...
	if($('#startDate').val() != ""  && document.getElementById('startDate') != null){
		document.getElementById("startDate").options[0].selected = 'selected';
	}
}
...
<script>
	$(document).on('change', '#subjectSelectContainerDiv select', function(){
		...
		if(subject !== ''){
			//--- API for get batch details ---//
			$.ajax({
				type : "POST",
				contentType : "application/json",
				url : "/acads/admin/api/getSessionDetails",   
				data : JSON.stringify(data),
				success : function(data) {
					var startDate = data.listOfStringDateData;
					if( !startDate ){
						alert("Unable to get Date And Time for selected Subject config, Kindly update selections and try again.");
						document.getElementById("subject").options[0].selected = 'selected';
						return;
					}
					...
					var labelForDate = "<label for='startDate'>Start Date</label>";
					var selectElementTopPart = "<select id='startDate' name='startDate' placeholder='Select Start Date' type='text' class='form-control' required>" 
					var selectElementBottomPart = " </select>";
					options = ""; 
					//--- Start ---//
					for(let i = 0; i < startDate.length; i++){
						options = options + "<option value='" + startDate[i] + "'> " + startDate[i] + " </option>";
					}
					var htmlToAppend = labelForDate + selectElementTopPart + " <option selected value=''> Select Start Date </option> " + options + selectElementBottomPart
					$('#dateSelectContainer').html( htmlToAppend );
