...
<body class="inside" onload="encodeURL();">

<%

ArrayList<VideoContentAcadsBean> videoContentList =  (ArrayList<VideoContentAcadsBean>)session.getAttribute("videoContentList");
...
<%@ include file="header.jsp"%>
    ...
    <section class="content-container login">
       ...
       <div class="row"><legend>Learning Resources for ${subject}</legend></div>
			...
			<table class="table table-striped " id="dataTable" style="font-size:12px">
							    ...
							    <% try{ 
							    	if(noOfVideoContents>0){
							    		int count=0;
							    		for(VideoContentAcadsBean video:videoContentList){
							    %>
							    	<tr>
							    		<td>
							    			
							    			<a href="/acads/admin/watchVideos?id=<%=video.getId() %>" target="_blank">
							    			<i class="fa-regular fa-play-circle" style="font-size:18px;" aria-hidden="true"></i>
							    			</a>
							    		</td>
							    		<td><%=video.getSubject() %> - <%=video.getFileName() %>  </td>
							    		<td>&ensp;</td>
							    		<td>&ensp;</td>
							    		<td><%=video.getTrack() %></td>
							    		<td><%=video.getSessionDate() %></td>	
							    		<td><%=video.getStartTime() %></td>		
							    		<td>Video</td>				    		
							    		<%if(roles.indexOf("Acads Admin") != -1){ %>
							    		<td> </td>
							    		<td>  </td>
							    		<%} %>
							    		<td>
							    			<a href="/acads/admin/watchVideos?id=<%=video.getId() %>" target="_blank">Watch</a>
							    			 <h4>	&nbsp; / &nbsp;	</h4>
							    			<!-- 
								...
								<c:forEach var="contentFile" items="${contentList}" varStatus="status">
							        <tr>
										...
										<td width="50%"><c:out value="${contentFile.activeDate}"/></td>
										<td> </td>
										<td> </td>
										<td> </td>
										<td>
											<c:choose>
												...
												<c:when
													test="${fn:endsWith(contentFile.previewPath, '.zip') || fn:endsWith(contentFile.previewPath, '.rar')}">
										...
										<td> -->
										<c:choose>
										<c:when test="${fn:endsWith(contentFile.previewPath, '.pdf') || fn:endsWith(contentFile.previewPath, '.PDF')
										...
										<c:url value="/admin/deleteContent" var="deleteurl">
										...
										<c:url value="/admin/editContent" var="editurl">
										  <c:param name="id" value="${contentFile.id}" />
										  <c:param name="subjectCodeId" value="${subjectCodeId}" />
										</c:url>
		...
		<c:url value="viewLastCycleRecordings" var="lastCycleRecordingsUrl">
		  <c:param name="subjectCodeId" value="${subjectCodeId}" />
		  <c:param name="month" value="${month}" />
		  <c:param name="year" value="${year}" />
		</c:url>
		...
		<%if(roles.indexOf("Faculty") != -1 ){ %>
			...
			<button id="edit" name="edit" class="btn btn-primary" formaction="viewApplicableSubjectsForFaculty">Back To Subject List</button>
		<%} %>
		...
		<button id="edit" name="edit" class="btn btn-primary" formaction="${lastCycleRecordingsUrl}">Last Academic Cycle Session Recordings</button>
		</div>
		</form>
		</div>
...
function encodeURL(){
	...
	var subject = url.substring(url.indexOf('=')+1);
	console.log(subject);
	console.log(encodeURIComponent(subject));
	/* window.location ='/acads/updateContent?subject='+encodeURIComponent(subject); */
	return false;
                    ...
                    <div class="sz-main-content-inner">
              				...
              				<div class="sz-content-wrapper examsPage">
              						...
              						<div class="sz-content">
										...
										<h2 class="red text-capitalize">Learning Resources for ${subject}</h2>
