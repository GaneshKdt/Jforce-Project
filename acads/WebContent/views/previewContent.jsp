										...
										<div class="clearfix"></div>
			              					...
			              					if("PDF".equals(request.getParameter("type"))){ %>
												...
												<%-- 	<iframe id="frameWindow" class="embed-responsive-item" src="<spring:eval expression="@propertyConfigurer.getProperty('CONTENT_PREVIEW_PATH')" /><%=request.getParameter("previewPath") %>?id=<%=Math.random() %>#toolbar=0"></iframe>
												 --%>
												 <iframe id="frameWindow" class="embed-responsive-item inner" src='<spring:eval expression="@propertyConfigurer.getProperty('BASE_URL_ACADS_STATIC_RESOURCES')" />pdfjsViewerX/webX/viewer.html?file=<spring:eval expression="@propertyConfigurer.getProperty('CONTENT_BASE_PATH')" /><%=request.getParameter("previewPath") %>#zoom=100'></iframe>
