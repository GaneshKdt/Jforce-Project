										...
										<c:if test="${action == 'postQueries' }">
											...
											<form:form  action="postQuery" method="post" modelAttribute="sessionQuery">
												<fieldset>
													...
													<form:hidden path="sessionId" value="${session.id }"/>
													<form:hidden path="programSemSubjectId" value="${sessionQuery.programSemSubjectId }"/>
													<div class="clearfix"></div>
													<div class="form-group">
													...
													<div class="form-group">
															<form:textarea path="query" id="query" maxlength="500" class="form-control" onchange="askToEnterQuery()" placeholder="The response for the query posted here will be provided within 48 working hours (depending on the query)." cols="50" required="required"/>
													</div>
											...
											<div class="panel-content-wrapper">
													...
													please approach the academic coordinators. Details of the same are as follows:</p>
													<div id="accordion3" class="table-responsive">
													<h4>Contact Details of Academic Coordinators. Click Here <i class="fa-regular fa-square-plus"></i></h4>
														<table class="table table-striped" style="font-size:12px">
															<thead>
																<tr>
																	<th>University Regional Office</th>
															...
															<tbody>
																...
																<tr>
																	<td>Chandigarh</td>
																	<td>Academic Coordinator</td>
																	<td>acchandigarh@nmims.edu</td>
																</tr>
																
																<tr>
																	<td>Indore</td>
																	<td>Academic Coordinator</td>
																	<td>acindore@nmims.edu</td>
																</tr>
															</tbody>
														</table>
													</div>
										...
										<c:if test="${not empty myQueries}">
											<div class="panel-content-wrapper">
												...
												<c:forEach var="queryAnswer" items="${myQueries}" varStatus="status">
													...
													<c:if test="${queryAnswer.isAnswered == 'Y' }"><i class="fa-solid fa-square-check"></i></c:if>
													<c:if test="${queryAnswer.isAnswered == 'N' }"><i class="fa-solid fa-hourglass-start"></i></c:if>
        ...
        function askToEnterQuery()
        {
		  ...
		  $("#queryType").on("change", function () {
			    ...
			    if (this.value == "Academic") {
			    	$("#query").attr('placeholder','All Course Content or Session Content related queries only.');
			    } else if(this.value == "Technical") {
			    	$("#query").attr('placeholder','For all other queries, which are NOT related to Course Content or Session Content.');
			    }else{
			        $("#query").attr('placeholder','The response for the query posted here will be provided within 48 working hours (depending on the query).');
			    }
