...
<%@ include file="messages.jsp"%>
   ...
   <section class="content-container login" style="height:550px;background-color:#fff;">
				   	...
				   	<table class="table table-striped table-hover">
				   		<tbody>
				   			...
				   			<%-- 	<td><fmt:formatDate  pattern="dd-MMM-yyyy"  value="${parsedDate}"/>, ${session.startTime}</td> --%>
				   				<input type="hidden" id="myField" value="${session.date}T${session.startTime}+05:30" />
				   					<td id="dateTime" ></td>
				   			</tr>
				   ...
				   <%if(roles.indexOf("Acads Admin") != -1){ %>
					   ...
					   <table class="table table-striped table-hover">
			   				...
			   				<tbody>
								...
								<!-- Added for common session -->
			   					<c:if test="${not empty commonProgramList.program}">
			   						<tr>
			   							<td><c:out value="${commonProgramList.program}" /></td>
			   							<td><c:out value="${commonProgramList.sem}" /></td>
			   							<td><c:out value="${commonProgramList.prgmStructApplicable}" /></td>
			   						</tr>
			   					</c:if>
			   				</tbody>
		   				</table>
					...
					<a data-toggle="collapse" href="#instructions" aria-expanded="false" aria-controls="instructions"><i class="fa-regular fa-square-plus" aria-hidden="true"></i> Instructions to attend session:</a>
					<div id="instructions" class="collapse">
						<ul >
						 ...
						 <!--  <li >If you are unable to join session using 'Attend Session' button above, then please use direct URL 
						  ...
						  <li ><a href="#" class="<%= disableLink %>" onClick="window.open('/acads/resources_2015/LiveSessionGuide.pdf')">Download User Guide to Attend Session</a></li>  
						</ul>
					</div>
				...
				<c:url value="${SERVER_PATH}acads/admin/viewAddSessionPollsForm" var="viewAddSessionPollsUrl">
				  <c:param name="id" value="${session.id}" />
				</c:url>
				
				<!-- Added For Poll Result Page -->
				<c:url value="${SERVER_PATH}acads/admin/viewSessionPollsResultsForm" var="viewSessionPollsResultsUrl">
					<c:param name="id" value="${session.id}" />
				</c:url>
					...
					<div class="controls" style="padding-bottom: 5px; padding-left: 15px;">
						...
						<%if((userId.startsWith("77") || userId.startsWith("79"))/*  && !"77215000851".equals(userId)  */){//Student %>
									...
									<c:if test="${session.noOfParallelSession > 1}">
										<div class="form-group">
										    <c:if test="${session.sessionName ne 'Guest Lecture'}">
												<a  class="btn btn-primary btn-sm " href="${attendUrl}" target="_blank"><i class="fa-solid fa-headphones fa-lg"></i> Attend Any Available Session</a>&nbsp;
											</c:if>
											<c:if test="${session.sessionName eq 'Guest Lecture'}">
												<a  class="btn btn-primary btn-sm " href="#" onClick="return showMessage();"><i class="fa-solid fa-headphones fa-lg"></i> Attend Any Available Session</a>&nbsp;
											</c:if>
										</div>
									</c:if>
								</c:if>
							...
							<c:if test="${enableAttendButton != 'true' && sessionOver != 'true'}">
								<c:choose>
									<c:when test="${isSessionAccess == 'true'}">
										<div class="meetingButtons">
											<a  class="btn btn-primary btn-sm <%= disableLink %>" href="#" onClick="return showAlert();"><i class="fa-solid fa-headphones fa-lg"></i> Attend Session</a> &nbsp;
										</div>
									</c:when>
									<c:otherwise>
										<div class="meetingButtons">
								...
								<div class="meetingButtons">
									<a  class="btn btn-primary btn-sm <%= disableLink %>" href="#" onClick="return showLRAlert();" target="_blank"><i class="fa-regular fa-play-circle fa-lg"></i> View Session Recordings</a> &nbsp;
								</div>
							</c:if>
							
							<c:if test="${showQueryButton == 'true' && session.isCancelled ne 'Y' }">
								<div class="meetingButtons">
									<a  class="btn btn-primary btn-sm <%= disableLink %>" href="${postQueryUrl}" target="_blank"><i class="fa-solid fa-circle-question fa-lg"></i> Post A Query</a> &nbsp;
								</div>
								<div class="meetingButtons">
									<a  class="btn btn-primary btn-sm <%= disableLink %>" href="${myQueriesUrl}" target="_blank"><i class="fa-solid fa-circle-question fa-lg"></i> My Queries</a> &nbsp;
								</div>
								...
								<c:if test="${not empty session.facultyId}">
									<c:choose>
										<c:when test="${videoId ne 0 }">
											<div class="meetingButtons">
												<a  class="btn btn-primary btn-sm <%= disableLink %>" href="${learningResourcesUrl}" target="_blank"><i class="fa-regular fa-play-circle fa-lg"></i> 
										...
										<c:otherwise>
											<div class="meetingButtons">
												<a  class="btn btn-primary btn-sm disabled  <%= disableLink %>" href="${learningResourcesUrl}" target="_blank"><i class="fa-regular fa-play-circle fa-lg"></i> 
												View Session Recordings by Prof. ${mapOfFacultyIdAndFacultyRecord[session.facultyId].fullName} </a> &nbsp;
								...
								<c:if test="${not empty session.altFacultyId}">
									<c:choose>
										<c:when test="${altVideoId ne 0 }">
											<div class="meetingButtons">
												<a  class="btn btn-primary btn-sm <%= disableLink %>" href="${learningResourcesUrlAlt}" target="_blank"><i class="fa-regular fa-play-circle fa-lg"></i> 
												View Session Recordings by Prof. ${mapOfFacultyIdAndFacultyRecord[session.altFacultyId].fullName} </a> &nbsp;
											</div>
										</c:when>
										<c:otherwise>
											<div class="meetingButtons">
												<a  class="btn btn-primary btn-sm disabled  <%= disableLink %>" href="${learningResourcesUrlAlt}" target="_blank"><i class="fa-regular fa-play-circle fa-lg"></i> 
												View Session Recordings by Prof. ${mapOfFacultyIdAndFacultyRecord[session.altFacultyId].fullName} </a> &nbsp;
												(The Session recording is not available)
											</div>
										</c:otherwise>
								...
								<c:if test="${not empty session.altFacultyId2}">
									<c:choose>
										<c:when test="${alt2VideoId ne 0 }">
											<div class="meetingButtons">
												<a  class="btn btn-primary btn-sm <%= disableLink %>" href="${learningResourcesUrlAlt2}" target="_blank"><i class="fa-regular fa-play-circle fa-lg"></i> 
												View Session Recordings by Prof. ${mapOfFacultyIdAndFacultyRecord[session.altFacultyId2].fullName} </a> &nbsp;
											</div>
										</c:when>
										<c:otherwise>
											<div class="meetingButtons">
												<a  class="btn btn-primary btn-sm disabled  <%= disableLink %>" href="${learningResourcesUrlAlt2}" target="_blank"><i class="fa-regular fa-play-circle fa-lg"></i> 
												View Session Recordings by Prof. ${mapOfFacultyIdAndFacultyRecord[session.altFacultyId2].fullName}</a> &nbsp;
												(The Session recording is not available)
											</div>
										</c:otherwise>
								...
								<c:if test="${not empty session.altFacultyId3}">
									<c:choose>
										<c:when test="${alt3videoId ne 0 }">
											<div class="meetingButtons">
												<a  class="btn btn-primary btn-sm <%= disableLink %>" href="${learningResourcesUrlAlt3}" target="_blank"><i class="fa-regular fa-play-circle fa-lg"></i> 
												View Session Recordings by Prof. ${mapOfFacultyIdAndFacultyRecord[session.altFacultyId3].fullName} </a> &nbsp;
											</div>
										</c:when>
										<c:otherwise>
											<div class="meetingButtons">
												<a  class="btn btn-primary btn-sm disabled  <%= disableLink %>" href="${learningResourcesUrlAlt3}" target="_blank"><i class="fa-regular fa-play-circle fa-lg"></i> 
												View Session Recordings by Prof. ${mapOfFacultyIdAndFacultyRecord[session.altFacultyId3].fullName} </a> &nbsp;
												(The Session recording is not available)
											</div>
										</c:otherwise>
							...
							<div class="col-xs-3 column">
								<c:choose>
									<c:when test="${isSessionAccess == 'true'}">
										<span title="Live Subject" style="font-size: 45px;">
											 <i class="fa-regular fa-eye"></i>
										</span>
									</c:when>
									<c:otherwise>
										<span title="Recorded Subject" style="font-size: 45px;">
						...
						<%if(roles.indexOf("Acads Admin") != -1 || roles.indexOf("Learning Center") != -1 || roles.indexOf("Corporate Center") != -1){ %>
							<c:if test="${session.isCancelled ne 'Y' }">
								<div class="meetingButtons">
									<a  class="btn btn-primary btn-sm" href="${joinUrl}"  target="_blank"><i class="fa-solid fa-headphones fa-lg"></i> Attend Session-For Students</a> &nbsp;
								</div>
							</c:if>
							<c:if test="${session.isCancelled eq 'Y' }">
							   <div class="form-group" style="color:#d2232a ;font-weight:bold;">
						...
						<%if(roles.indexOf("Faculty") != -1 || roles.indexOf("Acads Admin") != -1 || roles.indexOf("Acads Coordinator") != -1){ %>
								<div class="meetingButtons">
									<a  class="btn btn-danger btn-sm" href="${facultyFeedbackUrl}"  target="_blank"><i class="fa-regular fa-eye fa-lg"></i> View Feedback</a>	
								</div>
								<div class="meetingButtons">
									<a  class="btn btn-danger btn-sm" href="${viewQueryUrl}"  target="_blank"><i class="fa-solid fa-circle-question fa-lg"></i> View Queries</a>
								</div>
								<c:if test="${nosessionPolls eq true }">
								<div class="meetingButtons">
								...
								<div class="meetingButtons">
									<a  class="btn btn-primary btn-sm" href="${viewSessionPollsResultsUrl}" target="_blank"><i class="glyphicon glyphicon-align-left"></i> Session Polls Results</a> &nbsp;
								</div>
								</c:if>
							...
							<%if(roles.indexOf("Faculty") != -1 || roles.indexOf("Acads Admin") != -1){ %>
								...
								<c:if test="${session.isCancelled ne 'Y'}">
									...
									<c:if test="${not empty session.meetingKey }">
										<div class="meetingButtons">
											<a  class="btn btn-success btn-sm" href="${startZoomMeetingUrl}&type=0" target="_blank"><i class="fa-solid fa-video fa-lg"></i> Start Original Meeting for ${mapOfFacultyIdAndFacultyRecord[session.facultyId].fullName}</a>
										</div>
									</c:if>
									
									<c:if test="${not empty session.altMeetingKey }">
										<div class="meetingButtons">
											<a  class="btn btn-success btn-sm" href="${startZoomMeetingUrl}&type=1" target="_blank"><i class="fa-solid fa-video fa-lg"></i> Start Parallel Session 1 for ${mapOfFacultyIdAndFacultyRecord[session.altFacultyId].fullName}</a>
										</div>
									</c:if>
									<c:if test="${not empty session.altMeetingKey2 }">
										<div class="meetingButtons">
											<a  class="btn btn-success btn-sm" href="${startZoomMeetingUrl}&type=2" target="_blank"><i class="fa-solid fa-video fa-lg"></i> Start Parallel Session 2 for ${mapOfFacultyIdAndFacultyRecord[session.altFacultyId2].fullName}</a>
										</div>
									</c:if>
									<c:if test="${not empty session.altMeetingKey3 }">
										<div class="meetingButtons">
											<a  class="btn btn-success btn-sm" href="${startZoomMeetingUrl}&type=3" target="_blank"><i class="fa-solid fa-video fa-lg"></i> Start Parallel Session 3 for ${mapOfFacultyIdAndFacultyRecord[session.altFacultyId3].fullName}</a>
										</div>
									</c:if>
								...
								<%if(roles.indexOf("Acads Admin") != -1){ %>
									<br/>
									<div class="meetingButtons">
										<a  class="btn btn-info btn-sm" href="${createParallelSessionUrl}&type=1"  ><i class="fa-solid fa-video fa-lg"></i> Create Parallel Session 1</a>
									</div>
									<div class="meetingButtons">
										<a  class="btn btn-info btn-sm" href="${createParallelSessionUrl}&type=2"  ><i class="fa-solid fa-video fa-lg"></i> Create Parallel Session 2</a>
									</div>
									<div class="meetingButtons">
										<a  class="btn btn-info btn-sm" href="${createParallelSessionUrl}&type=3"  ><i class="fa-solid fa-video fa-lg"></i> Create Parallel Session 3</a>
									</div>
									...
									<c:if test="${not empty session.altMeetingKey || not empty session.altFacultyId || not empty session.altHostKey}">
										<div class="meetingButtons">
											<a class="btn btn-danger btn-sm" href="${deleteParallelSessionUrl}&type=1" onClick="return confirm('Are you sure you want delete Parallel Session 1 ?')" >
												<i class="fa-solid fa-trash fa-lg"></i> Delete Parallel Session 1
											</a>
										</div>
									</c:if>
									...
									<c:if test="${not empty session.altMeetingKey2 || not empty session.altFacultyId2 || not empty session.altHostKey2}">
										<div class="meetingButtons">
											<a class="btn btn-danger btn-sm" href="${deleteParallelSessionUrl}&type=2" onClick="return confirm('Are you sure you want delete Parallel Session 2 ?')" >
												<i class="fa-solid fa-trash fa-lg"></i> Delete Parallel Session 2
											</a>
										</div>
									</c:if>
									...
									<c:if test="${not empty session.altMeetingKey3 || not empty session.altFacultyId3 || not empty session.altHostKey3}">
										<div class="meetingButtons">
											<a class="btn btn-danger btn-sm" href="${deleteParallelSessionUrl}&type=3" onClick="return confirm('Are you sure you want delete Parallel Session 3 ?')" >
												<i class="fa-solid fa-trash fa-lg"></i> Delete Parallel Session 3
											</a>
										</div>
									</c:if>
								...
								<br/>
									<div class="meetingButtons">
										<a  class="btn btn-primary btn-sm " href="${uploadSessionWiseVideoContentFormUrl}" target="_blank"> 
											<i class="fa-regular fa-play-circle fa-lg"></i> &nbsp; 
...
document.getElementById("dateTime").innerHTML = moment(document.getElementById("myField").value).format("DD-MMM-YYYY HH:mm:ss A");
</script>
