...
public class StudentSessionPlanController {
...
//		List<ContentBean> lRList = dao.getContentsForSessionPlanModule(id);
		//commented by Riya as it contains left join
...
//		List<ContentBean> finalLRList = new ArrayList<>();
		List<ContentAcadsBean> lRList = dao.getAllContentsForSessionPlanModuleAndSapId( id);
		HashMap<String,String> contentid_bookmark = dao.getBookmarksOfContent(sapId);
		lRList.forEach(f -> f.setBookmarked(contentid_bookmark.get(f.getId())));
