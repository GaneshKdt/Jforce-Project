									...
									<td>
										<c:if test="${not empty bean.previewPath}">
										    <a target="_blank" href="downloadFile?filePath=${bean.filePath}">Download</a> / 
										   	<a target="_blank" href="<spring:eval expression="@propertyConfigurer.getProperty('CONTENT_PREVIEW_PATH')" />${bean.previewPath}">View</a>
										</c:if>
									</td>
									...
									<td> 
										...
										<c:choose>
											...
											<c:when test="${searchType eq 'all' }">
												<a href="${editurlForSingleMapping}" title="Edit"><i class="fa fa-pencil-square-o fa-lg"></i></a>&nbsp;	
						 						<a href="${deleteurlForSingleMapping}" title="Delete" onclick="return confirm('Are you sure you want to delete this record? ')"><i class="fa fa-trash-o fa-lg"></i></a>
											</c:when>
										</c:choose>
