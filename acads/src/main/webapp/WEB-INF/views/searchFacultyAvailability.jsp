...
<jsp:param value="Search Faculty Availability Dates" name="title" />
</jsp:include>
...
<%@ include file="header.jsp"%>
    ...
    <section class="content-container login">
        ...
        <div class="row"><legend>Search Faculty Availability Dates</legend></div>
        <%@ include file="messages.jsp"%>
		
		<form:form  action="searchFacultyAvailability" method="post" modelAttribute="searchBean">
			<fieldset>
			...
			<div class="col-md-6 column">
					...
					<div class="form-group">
							<form:input id="date" path="date" type="date" placeholder="Date" 
							class="form-control"  value="${searchBean.date}"/>
					</div>
					
					<div class="form-group">
						<label class="control-label" for="submit"></label>
						<button id="submit" name="submit" class="btn btn-large btn-primary" formaction="searchFacultyAvailability">Search</button>
	...
	<legend>&nbsp;Faculty Availability Dates<font size="2px"> (${rowCount} Records Found) &nbsp; </font></legend>
	...
	<table class="table table-striped" style="font-size:12px">
						...
						<c:forEach var="bean" items="${facultyAvailabilityList}" varStatus="status">
					        <tr>
								...
								<td><c:out value="${bean.time}" /></td>
								<td> 
						            <c:url value="editFacultyAvailability" var="editurl">
									  <c:param name="id" value="${bean.id}" />
									</c:url>
									<c:url value="deleteFacultyAvailability" var="deleteurl">
									  <c:param name="id" value="${bean.id}" />
									</c:url>
	...
	<c:url var="firstUrl" value="searchFacultyAvailabilityPage?pageNo=1" />
	<c:url var="lastUrl" value="searchFacultyAvailabilityPage?pageNo=${page.totalPages}" />
	<c:url var="prevUrl" value="searchFacultyAvailabilityPage?pageNo=${page.currentIndex - 1}" />
	<c:url var="nextUrl" value="searchFacultyAvailabilityPage?pageNo=${page.currentIndex + 1}" />
