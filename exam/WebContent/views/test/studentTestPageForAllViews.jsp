	...
	<div id="redBackgroundDiv">
		...
		<div id="contactAdminParentDiv">
			...
			<div id="contactAdminChildDiv">
				<i id="showAlertAndCloseWindowIcon" class="fa-solid fa-circle-exclamation"
													...
													<div class="card-header bg-white p-a-1"
														...
														<div id="prevNextDiv" class="">
															...
															<a onclick="markForReview(${testQuestion.id})"
																...
																class="fa-regular fa-bookmark" style="font-size: 25px;"></i>
															</a>
														</div>
														<br />
															...
															<div class="card-block p-a-2"
																...
																<div class="row">
																	...
																	<c:if test="${testQuestion.uploadType == 'pdf'}">			
																		...
																		<c:if test="${testQuestion.uploadType == 'zip'}">
																			  <a id="answerLink" href="" target="_blank">Click here to download the file <i class="fa fa-download" aria-hidden="true"></i></a>
																		</c:if>	
																	</div>
																	<!-- 	</div>  -->
																</div>
									...
									<c:forEach var="testQuestion" items="${testQuestions}"
										...
										<li class="list-group-item" style="cursor: pointer"
											...
											<div class="media">
												...
												<i id="bookmarkIconInQuestionStatus-${testQuestion.id}" class="fa-regular fa-bookmark" style = "display: inline ; font-size : 18px;" ></i>
												</div> 
												<div class="media-right">
...
-->
	...
	<script src="assets/js/CustomScriblePad.js"></script>
	 -->
	<script type="text/javascript" src="https://d1h28kdwpyiu28.cloudfront.net/assets/test/ckeditor_4-20-1_full/ckeditor/ckeditor.js"></script>
	<script type="text/javascript">
...
function testShwo(){
	...
	let dqEditorObject = {};				//object to hold CKEDITOR instances for Descriptive question answers with their respective element id as keys
	let editorInstance;						//variable used to store the CKEDITOR instance
	const textAreaConst = "textArea-";		//placeholder text textarea ids
	...
	$(document).ready(function() {
		 ...
		 try{
  	    ...
  	    if(localStorage.getItem("testSubmitted") === "Yes"){
       	...
       	window.location.href = "/exam/viewTestDetailsForStudentsForAllViews?userId=${userId}&id=${test.id}&message=''";
	  ...
	  var isReasonForRestartPopUpActive = false;
		...
		<c:if test = "${continueAttempt == 'Y'}">
			...
			<c:if test = "${testQuestion.type == 8}">
					...
					<c:choose>
				         <c:when test = "${testQuestion.uploadType == zip}">
				         	$('#answerLink').attr("href","${testQuestion.answer}");
				         </c:when>
		...
		////////////console.log('Entered testQuestion forEach')
			...
			/*								//Commented out as JQuery onChange does not work after CKEDITOR instance is initiated 
			...
			*/
		    ...
		    editorInstance = CKEDITOR.replace(textAreaConst + "${testQuestion.id}", {				//replaces textArea with a CKEditor instance
						    	language: "en",							//Default language - english
						    	customConfig: "",						//To completely avoid loading an external configuration file (good practice)
						    	//allowedContent = true,				//true disables the data filtering
						    	toolbarGroups: [
						    	...
						    	removePlugins: "sourcearea, elementspath, exportpdf, save-to-pdf",
								removeButtons: "About,CopyFormatting,RemoveFormat,Save,NewPage,Preview,Print,Templates," +
												"Cut,Copy,Paste,PasteText,PasteFromWord,Find,Replace,SelectAll," +
												"Scayt,ShowBlocks,Maximize,Styles,Format,Image,Flash,Smiley," +
												"PageBreak,Iframe,Link,Unlink,Anchor,Language,BidiRtl,BidiLtr,CreateDiv," +
												"Blockquote,Strike,Form,Checkbox,Radio,TextField,Textarea,Select," +
												"Button,ImageButton,HiddenField",
								width: "100%",							//sets the width of the editor using CSS unit (percent)
								height: "15em",							//sets the height of the editor using CSS unit (em)
								//toolbarCanCollapse: true,				//button to collapse toolbar
								//resize_minWidth: 300,					//Limiting the Width and Height of the editor
								//resize_minHeight: 400,
								//resize_maxWidth: 300,
								//resize_maxHeight: 400,
								//resize_dir: "both",					//allows both vertical and horizontal resizing
								resize_enabled: false					//false disables the resize handle
						    });

    		dqEditorObject[textAreaConst + "${testQuestion.id}"] = editorInstance;		//storing the ckeditor instance in an object with ID as key
		    
		    /*								//Commented out as JQuery bind does not work after CKEDITOR instance is initiated 
		...
		for(const editorId in dqEditorObject) {							//iterating through the object containing ckeditor instances
			let editorInstance = dqEditorObject[editorId];

			editorInstance.on("contentDom", function() {				//event fired when the editor content (its DOM structure) is ready
				editorInstance.removeMenuItem("cut");					//removes options from the context menu (right click menu)
		    	editorInstance.removeMenuItem("copy");
		    	editorInstance.removeMenuItem("paste");

		    	let editable = editorInstance.editable();				//editable provides editing related activities which dynamically get attached to editor instance
		    	editable.attachListener(editable, "cut", function(event) {
			        console.debug("Cut clipboard event performed by user: ", event);
			        event.cancel();										//cancels the event
			        event.data.preventDefault(true);					//prevents the behavior of the event (CKEDITOR dom event)
			        Swal.fire("Cut option is disabled !!","");			//display message to user
			    });

		    	editable.attachListener(editable, "copy", function(event) {
			        console.debug("Copy clipboard event performed by user: ", event);
			        event.cancel();										//cancels the event
			        event.data.preventDefault(true);					//prevents the behavior of the event (CKEDITOR dom event)
			        Swal.fire("Copy option is disabled !!","");			//display message to user
			    });
			});
			
			editorInstance.on("change", function(event) {
				const questionId = editorId.substring(textAreaConst.length);
				
				async function asyncCallToChangedText() {
					let saved = await changedText(questionId)
										.then(success, failure);
			...
			editorInstance.on("paste", function(event) {
				console.debug("Paste clipboard event performed by the user: ", event);
				event.cancel();											//disable paste event
		    	Swal.fire("Paste option is disabled !!","");			//display message to user
			});
		}
		...
		function answuerOftype4(questionDetails,actionTo){ //type 4 is of Descriptive
			...
			let dqEditorSelector = textAreaConst + dQuestionId;
			let dqData = dqEditorObject[dqEditorSelector].getData();
			...
			if(dqData.length == 0 ){
				...
				async function asyncCallToSaveDescriptiveAnswer(){
					let saved = await  saveDQAnswerInRedisAjax(dQuestionId,dqData,sapid,testId,questionDetails.type,false,actionTo)
						.then(success, failure)
				 		function success(data){
							//////////////console.log("In SUCCESS of AJAX CALL of Descriptive =====> ");
							$('#parentSpinnerDiv').hide();
		...
		//saveAnswerAjax start
		 function saveAnswerAjax(questionId,answer,sapid,testId,type,isSubQuestion,actionTo){
    		...
    		let response_time = new Date().getTime();
  			let apiUrl = window.location.origin+"/exam/m/addStudentsQuestionResponse";
		...
		//saveDQInRedis start
		 function saveDQAnswerInRedisAjax(questionId,answer,sapid,testId,type,isSubQuestion,actionTo){
    		...
    		let resquest_time = new Date().getTime();
    		let response_time = new Date().getTime();
  			let apiUrl = window.location.origin+"/timeline/api/studentTest/saveDQAnswerInCache";
    		
  			$.ajax({
    			type : 'POST',
    			url : '/timeline/api/studentTest/saveDQAnswerInCache',
    			data: JSON.stringify(body),
                contentType: "application/json",
                dataType : "json",
                timeout : 10000,
		...
		//saveDQInRedis end
			...
			//deleteAnswerBySapIdQuestionId start
			 function deleteAnswerBySapIdQuestionId(questionId,sapid){
	    		...
	    		$.ajax({
	    			type : 'POST',
	    			url : '/exam/deleteAnswerBySapIdQuestionId',
	    			data: JSON.stringify(body),
	                contentType: "application/json",
	                dataType : "json",
	                timeout : 10000,
...
$('#statusQuestion' + question).addClass('skipped_status');
		...
		function submitTest(message){
			...
			//apiLogCall start
			try{
	    		let apiUrl = window.location.origin+"/exam/startStudentTestPage_finishAssignmentButtonClicked";
		...
		function submitTestApiCall(message){
			...
			//////////////console.log("SUBMITING TEST got message: "+message);
			 ...
			 submitTestApiCallMade = true;
	    		...
	    		let resquest_time = new Date().getTime();
    			let response_time = new Date().getTime();
  				let apiUrl = window.location.origin+"/exam/m/saveStudentsTestDetails";
		...
		window.changedText = function changedText(textAreaId) {
			...
			let dqEditorTextSelector = textAreaConst + textAreaId;
			...
			let dqTextInput = dqEditorObject[dqEditorTextSelector].getData();			//gets the text data entered in the CKEditor instance
			...
			if(dqTextInput){
				...
				if(dqTextInput.length != 0 ){
					showTickForAnswerSaved(textAreaId);	
	          	}else if(dqTextInput.length == 0 ){
		...
		try{
			...
			if(allowedTimeAwayInMSecs === (totalTimeAway)){
				...
				showAlertAndCloseWindow("Test Window will close as you navigated away, Contact Course Coordinator on chat for any query.");
			}
				...
				try{
					if(showTestDetaisCount === 0){
						Swal.fire({
			                ...
			                + ' <br><b>Do not Refresh Or Close the page before Submission, this will finish your test and you will not be Allowed to Rejoin.</b> (Contact Course Coordinator on private chat immediately for any query)<br><br>'
		...
		//Move functions from private to here start
				...
				function submitReasonForLostFocus(){
					try{	
			    		...
			    		let resquest_time = new Date().getTime();
			    		let response_time = new Date().getTime();
			  			let apiUrl = window.location.origin+"/exam/saveReasonForLostFocus";
			    		
			    		
			    		$.ajax({
			    			type : 'POST',
			    			url : "saveReasonForLostFocus",
				...
				function showPageLostFocusAlert_OfSweatAlert(){
					...
					try{
			            ...
			            Swal.fire({
					                ...
					                text: "  You had navigated away from the test window, This will be reported to the faculty. Do not repeat this behaviour, Otherwise you will be marked for plagiarism. Inform Course Coordinator on chat immediately after test if there was a valid reason.",
					                icon: "warning",
					                showCancelButton: false,
		 ...
		 function showReportLostFocusDiv(){
	    	...
	    	try{
	    		...
	    		let apiUrl = window.location.origin+"/exam/iaStarted/navigatedAwayPopupShown";
		...
		//Move functions from private to here end
		  	  ...
		  	  <c:if test = "${continueAttempt == 'Y'}">
			  	...
			  	try{
			    	...
			    	try{
			    		let apiUrl = window.location.origin+"/exam/startStudentTestPage_pageLoaded";
						response_time_logPageLoadedEvent = new Date().getTime();
						response_payload_size_logPageLoadedEvent = 0 ;
			  			asyncApiLogAjaxCall(sapid,apiUrl,resquest_time_logPageLoadedEvent,response_time_logPageLoadedEvent,"Success","",response_payload_size_logPageLoadedEvent)
...
$('.input-file-btn').change(function (e) {
	...
	}else if($('#hiddenUploadType').val()=="zip"){
		acceptedSize =  70000000;
...
function saveMedia(myFile) {
	try{
		let resquest_time = new Date().getTime();
		let response_time = new Date().getTime();
		let apiUrl = window.location.origin+"/exam/m/uploadTestAnswerFile";
		...
		formData.append('image', myFileInput); 
		 $.ajax({
			url : '/exam/m/uploadTestAnswerFile?testId=${test.id}&userId=${userId}',
			data : formData,
			processData : false,
			contentType : false,
			type : 'POST',
