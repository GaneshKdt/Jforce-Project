			...
			<div style="background-color: #ECE9E7;"class="">
				...
				<div class="">
					...
					<div class="layout-container container">
						...
						<h2  style="color:grey; margin-left:10px;" class="text-capitalize">Internal Assessments Test For ${test.subject}</h2>
	...
	<div id="parentSpinnerDiv">
		...
		<div id="childSpinnerDiv">
			<i class="fa fa-refresh fa-spin"
				...
				<br>
				<span id="loadingSpinnerText"></span>
		</div>
											...
											<div class="col-md-12 col-sm-12 col-xs-12">
												...
												<div class="card">
													<div class="card-block center">
														...
														<small class="text-muted-light">Proctoring Status</small>
														<h4 class="m-b-0">
															<strong >
															
															<c:if test="${test.proctoringEnabled == 'Y' }">
																Proctoring Enabled
															</c:if>
															<c:if test="${test.proctoringEnabled == 'N' }">
																Proctoring Disabled
															</c:if>
...
<%-- 							<div class="row">
							...
							<c:if test="${ (test.maxAttempt > studentsTestDetails.attempt) && (continueAttempt != 'Y')}">
								<a href="#" title="Launch Assignment" id="goToTest"
									class="btn btn-primary"> Launch Assignment  <span class="">( ${test.remainingTime} mins left )</span></a>
							</c:if>
							
							<c:if test="${ (test.maxAttempt > studentsTestDetails.attempt) && (continueAttempt == 'Y')}">
								<a href="#" title="Continue Test" id="goToTest"
									class="btn btn-primary"> Continue Assignment <span class="">( ${test.remainingTime} mins left )</span> </a>
							</c:if>
							</c:if>
				...
				.ready(
						function() {
							...
							$("#goToTest")
									.click(
											function(e) {
												...
												showLoadingSpinner("Checking If Test Already Open, Please Wait...");
												
												 checkIfTestAlreadyOpen()
							...
							function launchAssignment(){
								...
								window.open('/exam/assignmentGuidelinesForAllViews?testIdForUrl=${testIdForUrl}&sapidForUrl=${sapidForUrl}&consumerProgramStructureIdForUrl=${consumerProgramStructureIdForUrl}', '', 'resizable=yes,status=no,location=no,toolbar=no,menubar=no,fullscreen=yes,scrollbars=yes,dependent=no,left=0,top=0,width='+w+',height='+h+''); 
								goToGuidelinesPageButtonClicked();
							}
							...
							function launchAssignmentForMobile(){
								...
								window.location.href = "/exam/assignmentGuidelinesForAllViews?testIdForUrl=${testIdForUrl}&sapidForUrl=${sapidForUrl}&consumerProgramStructureIdForUrl=${consumerProgramStructureIdForUrl}";
								goToGuidelinesPageButtonClicked();
							}
							...
							function showRefreshPopup(){
								...
								try{
						            ...
						            Swal.fire({
												...
												"<b>For any query contact course coordinator on chat.</b>",
								                icon: "warning",
							...
							//Code for checkIfTestAlreadyOpen start
						  	function checkIfTestAlreadyOpen() {
										 var promiseObj = new Promise(function(resolve, reject){
											
											 message_broadcast_checkIfTestAlreadyOpen({'command':'isIAPageOpen', 'uid': (new Date).getTime()+Math.random()})
											 timeoutObj = setTimeout(function(){ checkResponseFromWindow() } , 3000);

										 })
										 return promiseObj;
							 }	
							
							function checkResponseFromWindow(){
								if(noOfIAPagesResponses == 0){
									launchIAPagePopup()
								}else{
									showTestAlreadyOpenPopup();
								}	
							...
							function showTestAlreadyOpenPopup(){
								Swal.fire("IA Test Page Already Open In Other Window.", " Kindly close all open windows/tabs and retry joining the IA. ");
							}
							
						  	try{

						  		 $(window).on('storage', message_receive_checkIfTestAlreadyOpen);
						  	}catch(listenerOnStorageError){
								 console.log('IN catch() > listenerOnStorageError : ',listenerOnStorageError);
							}	 
						  		// use local storage for messaging. Set message in local storage and clear it right away
						  		// This is a safe way how to communicate with other tabs while not leaving any traces
						  		//
						  		function message_broadcast_checkIfTestAlreadyOpen(message)
						  		{
						  			 console.log('IN message_broadcast_checkIfTestAlreadyOpen > message : ',message);
										
						  			try{
						  				noOfIAPagesResponses = 0; 
						  				
							  		    localStorage.setItem('messageIsIAPageAlreadyOpen',JSON.stringify(message));
							  		    localStorage.removeItem('messageIsIAPageAlreadyOpen');
							  		}catch(messageIsIAPageAlreadyOpenError){
										 console.log('IN catch() > messageIsIAPageAlreadyOpenError : ',messageIsIAPageAlreadyOpenError);
									}
							  	}
						  		...
						  		function message_receive_checkIfTestAlreadyOpen(ev)
						  		{
						  			...
						  			try{
							  		    if (ev.originalEvent.key!='messageOfIAPageLoaded') return; // ignore other keys
							  		    var message=JSON.parse(ev.originalEvent.newValue);
							  		    if (!message) return; // ignore empty msg or msg reset

							  		    // here you act on messages.
							  		    // you can send objects like { 'command': 'doit', 'data': 'abcd' }
							  		    if (message.command == 'IAPageIsLoaded'){
							  		    	//alert(message.uid);
							  		    	noOfIAPagesResponses = noOfIAPagesResponses + 1;
							  		    } 
							  		    // etc.
							  		}catch(messageIsIAPageAlreadyOpenError){
										 console.log('IN catch() > messageIsIAPageAlreadyOpenError : ',messageIsIAPageAlreadyOpenError);
						  	...
						  	function showLoadingSpinner(spinnerMessage){
						  		$("#loadingSpinnerText").text(spinnerMessage);
						  	...
						  	function logPageLoadedEvent(){
						  		...
						  		console.log("logPageLoadedEvent() ==>");
				      			//apiLogCall start
				      			
					        	try{
					        		let apiUrl = window.location.origin+"/exam/viewTestDetailsForStudentsForAllViews_pageLoaded";
									...
									console.log("Error in logPageLoadedEvent() catch() :"); 
									console.log(err); 
								}
				      			//apiLogCall end
						  	}
						  	function goToGuidelinesPageButtonClicked(){
						  		 		
				      			//apiLogCall start
				      			
					        	try{
					        		let apiUrl = window.location.origin+"/exam/assignmentGuidelines_goToLinkClicked";
									response_time = new Date().getTime();
					    			response_payload_size =  0 ;
					      			asyncApiLogAjaxCall(sapid,apiUrl,resquest_time,response_time,"Success","",response_payload_size)
