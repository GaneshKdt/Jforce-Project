  ...
  <div class="tab-content">
    <div id="home" class="tab-pane fade in active">
		...
		<c:forEach var="answer" items="${answersToBeEvaluated}"
			...
			<c:choose>
				<c:when test="${fn:startsWith(answer.answer, 'https://') && answer.type eq 4}">
					<c:set var="answerUploadType" value="pdf" />
				</c:when>
				<c:otherwise>
					<c:set var="answerUploadType" value="${answer.uploadType}" />
				</c:otherwise>
			</c:choose>
			<div class="container" onClick="loadIframeURL(${answer.id}, '${answerUploadType}', '${fn:replace(fn:replace(answer.answer, '\"', ' '), '\'', ' ')}')" >
			      ...
			      <div id="collapse${answer.id}"  class="panel-collapse collapse">
			        <div class="panel-body">
			        	<c:if test="${answer.type != 8}">
				        	...
				        	<div class="well" >
				        		...
				        		<div class="container">
				        			<c:choose>
										<c:when test="${answerUploadType eq 'pdf'}">
											<div class="row">
			         ...
			         <span title="Toggle Fullscreen" id="go-button"
					        	...
					        	<div class="well" >					        	
						        		...
						        		<c:if test="${answer.url ne ''}">
						        		 	...
						        		 	Question Attachment : <a href="${answer.url}" style="white-space: pre-wrap;" target="_blank">Question file (please click to view the file) </a>
					        		...
					        		<div class="container" >
										...
										<c:if test="${answer.uploadType == 'mp4' }">
										 	<embed id="answerLink" src="${answer.answer}" showcontrols="true" 
										 		style="width: 100%; min-height: 500px;" type="video/webm" ></embed>
										</c:if>
										<c:if test="${answer.uploadType == 'zip' }">
											 <a id="answerLink" target="_blank" href="${answer.answer}"><strong>
											 	${fn:substringBefore(fn:substringAfter(answer.answer, "submissions/"), ".zip")} 
											 		(click to download <i class="fa fa-download" aria-hidden="true"></i>)</strong></a>
										</c:if>
									</div>
					        	</div>
				        	</div>
			        ...
			        <div class="panel-footer">
			        	...
			        	action="saveTestAnswerEvaluation" method="post" modelAttribute="answerBean">
							...
							<form:hidden path="sapid" value="${answer.sapid}" />
							...
							<div class="row">	
									...
									<div class="col-md-4 col-sm-6 col-xs-12 column">
										<div class="form-group">	
											...
											<form:select id="${answer.sapid}-${answer.testId}-${answer.attempt}-${answer.questionId}-marks" path="marks" 
												type="text" placeholder="select marks" class="form-control" required="required" > 
													    <form:option  value="0" >0</form:option>
													<form:option  value="0.5" >0.5</form:option>
													<form:option  value="1" >1</form:option>
													
													<%-- Using Scriplet as JSTL logic not working --%>
													
													<c:set var = "questionMarksJSTL" value = "${answer.questionMarks}" />
													<%
														try{
														
														Double questionMarksInDouble  = (Double)pageContext.getAttribute("questionMarksJSTL");
														//Double questionMarksInDouble = Double.parseDouble(questionMarksInString);
														
														int questionMarksRound = (int) Math.ceil(questionMarksInDouble);
														
														if(questionMarksInDouble > 1){
															for(int i = 2; i<=questionMarksRound; i++){
															
															%>
																<form:option  value="<%=i-0.5%>" ><%=i-0.5%></form:option>
														 	<%
																if(questionMarksInDouble >= i){
																	%>
									...
									<c:choose>
										<c:when test="${canFacultyEditIA == 'Y' }">
											<input id="nonevaluated-submit-button" type='button' class="btn btn-large btn-primary" value='Save'
												onClick="validateRemarksAndMarks(`${answer.sapid}-${answer.testId}-${answer.attempt}-${answer.questionId}`)" >
										</c:when>
										<c:otherwise>
											<button class="btn btn-large btn-primary disabled" disabled>Save</button>
										</c:otherwise>
									</c:choose>
    ...
    <div id="menu1" class="tab-pane fade">
		...
		<c:forEach var="answer" items="${answersEvaluated}"
			...
			<div class="container"  onClick="loadEvaluatedIframeURL(${answer.id}, '${answerUploadType}', '${fn:replace(fn:replace(answer.answer, '\"', ' '), '\'', ' ')}')">
			  <div class="panel-group">
			    <div class="panel panel-default">
			      <div class="panel-heading">
			        <h4 class="panel-title">
			      ...
			      <!--   <div class="row">
			        ...
			        <div class="panel-footer">
			        	...
			        	<form:form id="${answer.sapid}-${answer.testId}-${answer.attempt}-${answer.questionId}-evaluated-form"  
			        		action="saveTestAnswerEvaluation" method="post" modelAttribute="answerBean">
							<form:hidden path="id" value="${answer.id }"/> 
							<form:hidden path="testId" value="${answer.testId }"/>
							<form:hidden path="sapid" value="${answer.sapid}" />
							...
							<div class="row">	
									...
									<div class="col-md-4 col-sm-6 col-xs-12 column">
										<div class="form-group">	
											<form:label path="marks" for="applicableType">Select Marks To Be Assigned To Above Answer</form:label>
											<form:select  id="${answer.sapid}-${answer.testId}-${answer.attempt}-${answer.questionId}-evaluated-marks" 
												path="marks" type="text" placeholder="select marks" itemValue="${answer.marks}" 
												class="form-control" required="required" > 
													   
													  <%-- Using Scriplet as converting string to double JSTL not working --%>
													
													
													<c:set var = "questionMarksJSTL" value = "${answer.questionMarks}" />
													<c:set var = "answerMarksJSTL" value = "${answer.marks}" />
													<%
														try{
														
														Double questionMarksInDouble  = (Double)pageContext.getAttribute("questionMarksJSTL");
														Double answerMarksInDouble  = (Double)pageContext.getAttribute("answerMarksJSTL");
														
														int questionMarksRound = (int) Math.ceil(questionMarksInDouble);
														if(answerMarksInDouble.equals(0.0)){
 
																%>
																	<form:option  selected="selected" value="0.0" >0</form:option>
															 	<%
															}else{
																%>
																	<form:option  value="0.0" >0</form:option>
														 		<%
																
															}	
														
														if(answerMarksInDouble.equals(0.5)){
															%>
																	<form:option  selected="selected" value="0.5" >0.5</form:option>
															 	<%
															}else{
																%>
																	<form:option  value="0.5" >0.5</form:option>
														...
														if(answerMarksInDouble.equals(1.0)){
																%>
																	<form:option  selected="selected" value="1.0" >1</form:option>
															 	<%
														}else{
																%>
																	<form:option  value="1.0" >1</form:option>
														 		<%
														}	
														...
														if(questionMarksInDouble > 1){
															for(int i = 2; i<=questionMarksRound; i++){
																if(answerMarksInDouble.equals(((double) i-0.5))){
																	%>
																		<form:option  selected="selected" value="<%=i-0.5%>" ><%=i-0.5%></form:option>
																 	<%
																}else{
																	%>
																		<form:option  value="<%=i-0.5%>" ><%=i-0.5%></form:option>
																...
																if(questionMarksInDouble >= i){
																	if(answerMarksInDouble.equals(((double) i))){
																		%>
									...
									<div class="row">
										...
										<br>
										 ...
										 <textarea id="${answer.sapid}-${answer.testId}-${answer.attempt}-${answer.questionId}-evaluated-remarks" 
										 	name="remark" placeholder="Add Remark" type="textarea" class="form-group" rows="10" cols="80">${answer.remark}</textarea>
									...
									<c:choose>
										<c:when test="${canFacultyEditIA == 'Y' }">
											<input id="evaluated-submit-button" type='button' class="btn btn-large btn-primary" value='Save'
												onClick="validateRemarksAndMarks(`${answer.sapid}-${answer.testId}-${answer.attempt}-${answer.questionId}-evaluated`)" >
										</c:when>
										<c:otherwise>
											<button class="btn btn-large btn-primary disabled" disabled>Save</button>
										</c:otherwise>
    ...
    <div id="menu2" class="tab-pane fade">
		...
		<c:forEach var="answer" items="${copyCasesAnswers}"
			...
			<div class="container" onClick="loadCopyCaseIframeURL('${answer.id}', '${answerUploadType}', '${fn:replace(fn:replace(answer.answer, '\"', ' '), '\'', ' ')}')">
			  <div class="panel-group">
			    <div class="panel panel-default">
			      <div class="panel-heading">
			        <h4 class="panel-title">
				        		...
				        		<c:choose>
									<c:when test="${answerUploadType eq 'pdf'}">
										<div class="row">
				        		 		...
				        		 		<iframe id="answerLink"
				        		 			class="copyCaseIFrameClass${answer.id}"
											style="width: 100%; min-height: 500px;" 
											frameborder='0' webkitAllowFullScreen
											mozallowfullscreen allowFullScreen>
										</iframe>
							...
							<div class="row">	
									...
									<div class="row">
										...
										<br>
										 ...
										 <textarea id="remark" name="remark" placeholder="Add Remark" type="textarea" class="form-group" 
										 	rows="10" cols="80">${answer.remark}</textarea>
	...
	function loadIframeURL(answerId, answerUploadType, answerAnswer) {
		var iFrameClass = "iFrameClass" + answerId;
		
		//Checks if answerUploadType is not null or undefined and value equals to pdf
		if(answerUploadType != null && answerUploadType === "pdf") {
			var x = document.getElementsByClassName(iFrameClass);
			x[0].src = answerAnswer+"#zoom=100";
		}
	}
	
	function loadEvaluatedIframeURL(answerId, answerUploadType, answerAnswer) {
	...
	function loadCopyCaseIframeURL(answerId, answerUploadType, answerAnswer) {
		const iFrameClass = "copyCaseIFrameClass" + answerId;
		
		//Checks if answerUploadType is not null or undefined and value equals to pdf
		if(answerUploadType != null && answerUploadType === "pdf") {
			let x = document.getElementsByClassName(iFrameClass);
			x[0].src = answerAnswer+"#zoom=100";
		}
	}
	...
	function validateRemarksAndMarks(id){
		...
		if( marks == 0 ){
			...
			$('#action-modal-body-footer').html("<button type='button' class='btn btn-primary ' style='margin: 10px;' onclick=saveAnswerEvaluation(`"+id+
		...
		if(typeof(remarks) !== "undefined" && remarks !== null && remarks !== "" &&  marks != 0){
			saveAnswerEvaluation(id)
			return
		}
	}
	function saveAnswerEvaluation(id){
		console.debug('id: '+id+'-form')
