				...
				<div class="panel-body">
					...
					<div class="form-group">
						<button class="btn btn-large btn-primary"
							formaction="searchMarksForDissertation" id="submit" name="submit">Search</button>

						<c:if test="${searchList.size() > 0 }">
							<button class="btn btn-large btn-primary"
								formaction="transferDisserationToStaging" id="transfer"
					...
					<c:if test="${transferedStagingCount > 0 }">
						<a href ="/exam/admin/downloadDissertationResultQ7" target ="_blank">DownLoad Excel</a>
				</c:if>
				</div>
							...
							<div class="table-responsive">
								<table class ="dataTable" class="table table-striped table-hover tables">
														<thead>
															<tr>
																<th>SapId</th>															
																<th>Component A Score</th>
																<th>Component B Score
																<th>Component A Score Status</th>
																<th>Component B Score Status</th>													
															</tr>
														</thead>
														<tbody>
															<c:forEach var="marks" items="${searchList}">
																<tr>
																	...
																	<td><c:out value="${marks.component_a_score}" /></td>
																	<td><c:out value="${marks.component_b_score}" /></td>
																	<td><c:out value="${marks.component_a_status}" /></td>
																	<td><c:out value="${marks.component_b_status}" /></td>				
																</tr>
															</c:forEach>
														</tbody>
													</table>
