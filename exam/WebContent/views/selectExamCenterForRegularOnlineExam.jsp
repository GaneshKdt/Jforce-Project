...
StudentExamBean student = (StudentExamBean)session.getAttribute("studentExam");
	...
	String hasReleasedSubjects = (String)request.getAttribute("hasReleasedSubjects");
	String isBookingSeatAfterExamIsNotLive = (String)request.getAttribute("isBookingSeatAfterExamIsNotLive");
	int examFeesPerSubject = 600;
	int totalFeesForRebooking = 500;
	int totalFeesForRebookingWhenExamIsNotLive = 1000;
	int noOfSubjects = 0;
...
<html lang="en">
    ...
    <body>
										...
										<div class="clearfix"></div>
		              					<div class="panel-content-wrapper">
											...
											<form:form id="form1" action="makePaymentForm" method="post" modelAttribute="examCenter" >
												...
												<table class="table table-striped" style="font-size:12px">
													<thead>
														<tr> 
															...
															<th>Start Time</th>
															<th>End Time</th> -->
															<th id="center-city-column"> Select Exam Center City </th>
												...
												if("true".equals(hasReleasedSubjects)){ %>
													<input type="hidden"  name = "hasReleasedSubjects" value = "<%=hasReleasedSubjects%>"/>
													<input type="hidden"  name = "isBookingSeatAfterExamIsNotLive" value = "<%=isBookingSeatAfterExamIsNotLive%>"/>
												<%} %>
		...
		$('#selectPayu').click(function(){
			...
			// console.log("formation: " + formaction + " || payu");
			...
			$(document).ready(function() {
				...
				$(".center-select").each(function() {
			    	...
			    	if(totalOptions == 1) {
					    ...
					    $(".center-select").select2({disabled:'readonly'});
					    setHiddenFieldValue(selectId);
						$(this).trigger("change");
					    centerChanged(selectId);
					    var headerIndex = $( "th" ).index( $('#center-city-column') ) + 1
					    $('td:nth-child('+ headerIndex + '),th:nth-child('+ headerIndex + ')').hide();
			    	}
				})
