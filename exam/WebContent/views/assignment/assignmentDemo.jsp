				...
				<div class="sz-content-wrapper examsPage">
					...
					<div class="sz-content">
						<a href="/studentportal/${courseUrl}"
							...
							<h5 class="text-capitalize text-danger fw-bolder mt-3 mb-3">${assignmentFile.subject}</h5>
						</a>
									...
									<fmt:parseDate value="${assignmentFile.endDate}"
										...
										<c:if test="${endDate lt now}"> 
											<div id="ExpiredErrorMessages">
												<h5 class="text-capitalize text-danger fw-bolder mt-4 mb-3">Assignment Submission Deadline has Passed.</h5>
											</div>
										</c:if>
										<div class="clearfix"></div>
										<hr />
										<form action="editCompany" method="post">
											...
											<c:if test="${assignmentPaymentPending == 'Y' }">
												<div
													class="rounded bg-danger text-white pt-3 pb-3 pe-3 ps-3 fs-5">Assignment
											...
											<c:if test="${assignmentPaymentPending != 'Y' }">
												...
												<c:choose>
												 ...
												 <p class="rounded bg-success text-white pt-3 pb-3 pe-3 ps-3 fs-5">
												 <b>Status:</b> ${assignmentFile.status}</p>
												        </c:when>
												   <c:when test="${assignmentFile.status != 'Submitted'}">
												    <p class="rounded bg-danger text-white pt-3 pb-3 pe-3 ps-3 fs-5">
												    <b>Status:</b> ${assignmentFile.status}</p>
												   </c:when>
												  </c:choose>
											...
											<div class="control-group">
												...
												<div class="controls" id="controlsId">
													<c:if test="${endDate gt now}"> 
														<c:if test="${(assignmentPaymentPending) != 'Y' && (submissionAllowed) == 'Y'}">
															<c:if test="${assignmentFile.attempts < maxAttempts}">
																	<button id="edit" name="edit" class="btn btn-danger" onclick="return validateForm(${assignmentFile.attempts}, ${maxAttempts});"
														...
														<c:if test="${assignmentPaymentPending == 'Y'}">
															<a href="${selectAssignmentPaymentSubjectsFormUrl}"
																class="btn btn-danger">Proceed to Payment</a>
														</c:if>
													...
													%>
													 <c:if test="${(assignmentPaymentPending) != 'Y' && (submissionAllowed) == 'Y'}">
														<c:if test="${assignmentFile.attempts < maxAttempts}">
															<button id="edit" name="edit" class="btn btn-danger"
													...
													%>
													<a href="/exam/student/viewAssignmentsForm" title="Back to List" class="btn btn-dark" > Back to List</a>
													<!-- 
													<button id="cancel" name="cancel" class="btn btn-danger"
