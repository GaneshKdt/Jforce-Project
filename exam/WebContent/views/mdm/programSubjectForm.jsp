									...
									<div class="panel-body">
										<div class="column">
											...
											<div class="col-sm-3 column">
												...
												<form:select id="specializationName" path="specializationName" type="text"
													 placeholder="Select Specialization Name"  required="required"
													class="form-control" value="${programSubjectMappingBean.specializationName}">
													<form:option value="">Select Specialization Name</form:option>
													<form:option value="No Specialization">No Specialization</form:option>
													<form:options items="${specializationTypeList}"/>
											...
											<label>Is TimeBound ?</label>
												<form:select id="studentType" path="studentType" type="text" required="required" placeholder="Student Type"
												 class="form-control"   itemValue="${programSubjectMappingBean.studentType}" >
													...
													<form:option value=""> Select Status</form:option>
													<form:option value="TimeBound">Yes</form:option>
													<form:option value="Regular">No</form:option>
													
												</form:select>
											</div>
											<div class="col-sm-3 column">
												<label>Has IA?</label>
												<form:select id="hasIA" path="hasIA"
													type="text" 
													placeholder="Select IA Applicable"
													class="form-control"
													itemValue="${programSubjectMappingBean.hasIA}">
											...
											<div class="col-sm-3 column">
												...
												<form:input id="description" path="description"
													...
													value="${programSubjectMappingBean.description}" />
									...
									<table class="table table-striped table-hover tables"
										...
										<tbody>
											...
											<c:forEach var="programSubjectList"
												...
												<tr
													value="${programSubjectList.consumerType}~${programSubjectList.program}~${programSubjectList.subject}~${programSubjectList.prgmStructApplicable}~${programSubjectList.sem}~${programSubjectList.specializationName}">
													...
													<td><c:out value="${programSubjectList.specializationName}" /></td>
													<td><c:out value="${programSubjectList.hasIA}" /></td>
													<td><c:out value="${programSubjectList.studentType }"></c:out>
													<td><c:out value="${programSubjectList.description}"></c:out></td>
													<td></td>
		...
		$('.tables').Tabledit({
			...
			columns: {
			  ...
			  editable: [[4, 'sifySubjectCode'],
					  ...
					  [17, 'maxGraceMarks'],
					  [19, 'hasIA', '{"":"Select Status","Y": "Y", "N": "N"}'],
				//	  [20, 'studentType', '{"":"Select Status","TimeBound": "TimeBound", "Regular": "Regular"}' ],
					  [21,'description']
				//  [18, 'specializationName','{"No Specialization":"No Specialization","${specializationTypeList[0]}":"${specializationTypeList[0]}"}']
					  ]
			},
...
return; },
		...
		onAjax: function(action, serialize) {
			...
			serialize['sem'] = id[4]; 
			serialize['specializationName'] = id[5]; 
			serialize['lastModifiedBy'] = '${userId}';
			body = JSON.stringify(serialize)
			alert('test')
			console.debug('bodyy',body)
  		 $.ajax({
            type:"POST",
