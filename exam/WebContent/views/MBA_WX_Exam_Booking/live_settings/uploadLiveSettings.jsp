...
<body class="inside">
	...
	<section class="content-container login">
		<div class="container-fluid customTheme">
			<fieldset class="row"><legend>Upload MBA-WX Live Settings </legend></fieldset>
								...
								<div>
									...
									<div class="col-md-12 column">
										...
										PROGRAM_STRUCTURE | ACADS_YEAR | ACADS_MONTH | EXAM_YEAR | EXAM_MONTH | START_DATE | START_TIME | END_DATE | END_TIME
										 <br>
										<a href="<spring:eval expression="@propertyConfigurer.getProperty('BASE_URL_EXAM_STATIC_RESOURCES')" />resources_2015/templates/MBAWX_LIVE_SETTINGS_SAMPLE.xlsx" target="_blank">Download a Sample Template</a> <br>
									</div>
								</div>
								<br>
								<div class="row">
									<div class="col-md-6 column">
										<button id="submit" name="submit" class="btn btn-large btn-primary" formaction="uploadLiveSettings_MBAWX">
											Upload
										</button>
									</div>
								</div>
					...
					<div id="add-form" class="panel-collapse collapse" role="tabpanel" aria-labelledby="addForm">
						<div class="panel-body">
							<form:form modelAttribute="fileBean" method="post">
										...
										<div class="col-md-6 column">
											<div class="form-group">
												...
												<form:select path="program" id="program">
													...
													<form:option value="M.Sc. (AI & ML Ops)">M.Sc. (AI & ML Ops)</form:option>
													<form:option value="M.Sc. (AI)">M.Sc. (AI)</form:option>
													<form:option value="PC-DS">PC-DS</form:option>
													<form:option value="PD-DS">PD-DS</form:option>
													<form:option value="PDDM">PDDM</form:option>
												</form:select>
											</div>
										</div>
								...
								<div class="row">
									<div class="col-md-6 column">
										<a class="btn btn-large btn-danger" href="uploadLiveSettingsForm_MBAWX">
											Cancel
										</a>
										<button id="submit" name="submit" class="btn btn-large btn-primary" formaction="addLiveSettings_MBAWX">
											Update
										</button>
									</div>
								</div>
							 			...
							 			<tbody>
							 				<c:forEach var="liveSetting" items="${successList}" varStatus="loop">
							 					<tr>
													...
													<td><c:out value="${liveSetting.program}"/></td>
						 				...
						 				<c:forEach var="liveSetting" items="${liveSettingsList}" varStatus="loop">
						 					<tr>
												...
												<td><c:out value="${liveSetting.endTimeStr}"/></td>
						 						<td>
						 							<a 
							 							href="updateLiveSettingForm_MBAWX?Id=${liveSetting.id}" 
							 							style="color:#c72127;"
						 							>
						 								Update
					 								</a>
						 							<a 
							 							...
							 							id="${liveSetting.id}" 
							 							class="deleteLiveSettingFromList_MBAWX" 
							 							style="color:#c72127;"
						 							>
						 								Delete
				 									</a>
	...
	<script>
		$(document).ready( function () {
			...
			$(document).on('click',".deleteLiveSettingFromList_MBAWX",function(){
				...
				let id = $(this).attr("id");
				$.ajax({
					url:"deleteLiveSetting_MBAWX?id=" + id,
					method:"POST",
					success:function(response){
						if(response.status == "success"){
							alert(response.message);
							window.location.href = "uploadLiveSettingsForm_MBAWX";
						}else{
							alert(response.message);
						}
					},
