...
<%@ taglib prefix="form" uri="http://www.springframework.org/tags/form" %>
	...
	<jsp:param value="Apply Grace" name="title"/>
    </jsp:include>
    <head>
				...
				<div class="panel-body">
					...
					<div class="form-group">
						<button class="btn btn-large btn-primary"
							formaction="searchApplicateGraceStudents" id="submit" name="submit">Search</button>

						<c:if test="${graceStudent.size() > 0 }">
							<button class="btn btn-large btn-primary"
								formaction="dissertationApplyGrace" id="transfer"
								name="submit" formnovalidate>Apply Grace</button>
						</c:if>

					<a class="btn btn-large btn-primary" type="button" href = "dissertationQ8ExamResultCheckList" >Cancel</a>
				</div>
					
					<c:if test="${graceCount > 0 }">
						...
						<div class="column">
							<div class="table-responsive">
								<table class ="dataTable" class="table table-striped table-hover tables">
														...
														<tbody>
															<c:forEach var="grace" items="${graceStudent}">
																<tr>
																	<td><c:out value="${grace.sapid}" /></td>
																	<td><c:out value="${grace.component_c_score}" /></td>
																	<td><c:out value="${grace.isPass}" /></td>				
																</tr>
															</c:forEach>
														</tbody>
													</table>
