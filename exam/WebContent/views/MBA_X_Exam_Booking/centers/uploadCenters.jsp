...
<body class="inside">
	...
	<section class="content-container login">
		<div class="container-fluid customTheme">
			<fieldset class="row"><legend>Upload MBA-X Center list </legend></fieldset>
			...
			<form:form modelAttribute="fileBean" method="post"	enctype="multipart/form-data">
				<div class="panel-body">
					...
					<div class="col-md-12 column">
						...
						<a href="<spring:eval expression="@propertyConfigurer.getProperty('BASE_URL_EXAM_STATIC_RESOURCES')" />resources_2015/templates/sampleCenterMBAX.xlsx" target="_blank">Download a Sample Template</a> <br>
					</div>
				</div>
				<br>
				<div class="row">
					<div class="col-md-6 column">
						<button id="submit" name="submit" class="btn btn-large btn-primary" formaction="uploadCenters_MBAX">
							Upload
						</button>
						<a href = "autoGenerateSlots_MBAX" class="btn btn-large btn-primary">
							Generate Slots
						</a>
						<c:if test = "${ showApproveButton }">
							<a href = "approveUploadCenters_MBAX" id="approve" name="approve" class="btn btn-large btn-primary">
									...
									<tbody>
										<c:forEach var="center" items="${centersList}">
											<tr>
												...
												<td>
													<a 
														href="updateCenterForm_MBAX?centerId=${center.centerId}" 
														style="color:#c72127;"
													>
														Edit
													</a>
													<a 
														href="javascript:void(0)" 
														centerId="${center.centerId}" 
														class="deleteCenterFromList_MBAX" 
														style="color:#c72127;"
													>
														Delete
													</a>
	...
	<script>
		$(document).ready( function () {
			...
			$(document).on('click',".deleteCenterFromList_MBAX",function(){
				...
				$.ajax({
					url:"deleteCenter_MBAX?centerId=" + centerId,
					method:"POST",
					success:function(response){
						if(response.status == "success"){
							alert(response.message);
							window.location.href = "uploadCentersForm_MBAX";
						}else{
							alert(response.message);
						}
					},
			...
			$(document).on('click',".toggleCenterActive_MBAX",function(){
				...
				$.ajax({
					url:"toggleCenterActive_MBAX?centerId=" + centerId + "&active=" + active,
					method:"POST",
					success:function(response){
						if(response.status == "success"){
							alert(response.message);
