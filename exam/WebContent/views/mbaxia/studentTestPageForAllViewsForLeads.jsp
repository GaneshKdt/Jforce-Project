...
<body id="element" style="background-color: #ECE9E7;">
	...
	<div id="redBackgroundDiv">
		...
		<div id="contactAdminParentDiv">
			...
			<div id="contactAdminChildDiv">
				<i id="showAlertAndCloseWindowIcon" class="fa fa-exclamation-circle"
					style="font-size: 50px; color: grey;"></i><br>
				<span id="showAlertAndCloseWindowText"></span>
				<br>
	...
	<div id="pageContainer" class="">
										...
										<div class="card-block center"
											...
											<h4 class="text-primary m-b-0 "
												...
												<strong id="testTime"> <i
													id="bookmarkIcon-${testQuestion.id}" class="fa fa-clock-o"
		...
		<a href="#" title="Toggle Calculator" id="toggleCalculatorButton"><i id="toggleCalculatorButtonIcon" class="fa fa-calculator" style="font-size:24px"></i></a>
...
-->
	...
	<script src="<spring:eval expression="@propertyConfigurer.getProperty('BASE_URL_EXAM_STATIC_RESOURCES')" />assets/js/ckeditor/ckeditor.js"></script>
	<!-- 	
		...
		function closeTestWindow(){
			if(!isMobileCheck){
				try{
					...
					window.location.href = "/exam/viewTestDetailsForStudentsForAllViewsForLeads?userId=${userId}&id=${test.id}&message=''";
				}
			}else{
				window.location.href = "/exam/viewTestDetailsForStudentsForAllViewsForLeads?userId=${userId}&id=${test.id}&message=''";
		    }
		...
		//saveAnswerAjax start
		 function saveAnswerAjax(questionId,answer,sapid,testId,type,isSubQuestion,actionTo){
    		...
    		let response_time = new Date().getTime();
  			...
  			$.ajax({
    			type : 'POST',
    			url : '/exam/m/addStudentsQuestionResponseForLeads',
    			...
    			try{
	    			...
	    			response_payload_size = xhr ? xhr.length : 0 ;
	      			//asyncApiLogAjaxCall(sapid,apiUrl,resquest_time,response_time,status,error_message,response_payload_size)
	      			asyncApiLogAjaxCall(sapid,apiUrl,resquest_time,response_time,"Error","Eror in saving Answer : "+JSON.stringify(body)+JSON.stringify(xhr),response_payload_size)
      			}catch(err){
    			...
    			if(saveAnswerAjaxRetryCounter === 0 ){
					...
					try{
						...
						if(responseStatusMessage){
							if(responseStatusMessage.includes("TimeOver")){
		...
		//saveAnswerAjaxr end
			...
			//deleteAnswerBySapIdQuestionId start
			 function deleteAnswerBySapIdQuestionId(questionId,sapid){
	    		...
	    		$.ajax({
	    			type : 'POST',
	    			url : '/exam/deleteAnswerBySapIdQuestionIdForLeads',
	    			data: JSON.stringify(body),
	                contentType: "application/json",
	                dataType : "json",
	                timeout : 10000,
		...
		//logAutoSaveApiHit Start
				...
				//logAutoSaveApiHit start
				 function logAutoSaveApiHit(questionId,sapid){
		    		...
		    		$.ajax({
		    			type : 'POST',
		    			url : '/exam/m/logAutoSaveApiHitForLeads',
		    			data: JSON.stringify(body),
		                contentType: "application/json",
		                dataType : "json",
		                timeout : 10000,
		...
		//asyncApiLogAjaxCall start
		 function asyncApiLogAjaxCall(sapid,apiUrl,resquest_time,response_time,status,error_message,response_payload_size){
    		...
    		$.ajax({
    			...
    			//////////console.log("iN asyncApiLogAjaxCall AJAX eRROR");
...
$('#statusQuestion' + question).addClass('skipped_status');
		...
		function submitTestApiCall(message){
			...
			//////////////console.log("SUBMITING TEST got message: "+message);
			 ...
			 submitTestApiCallMade = true;
	    		...
	    		$.ajax({
	    			type : 'POST',
	    			url : '/exam/m/saveStudentsTestDetailsForLeads',
	    			data: JSON.stringify(body),
	                contentType: "application/json",
	                dataType : "json",
	                timeout : 10000,
		...
		try{
			...
			//asyncLostFocusAjaxCall start
			 function asyncLostFocusAjaxCall(dataToSend){
				...
				var urlForLostFocus = "";
				var apiNameForLostFocus = "/ltidemo/api/saveIATestLostFocusLogsForLeads";
				if(ENVIRONMENT === 'PROD'){
				...
				try{
					if(showTestDetaisCount === 0){
						Swal.fire({
			                ...
			                + ' (Test will start in 10 secconds, If not clicked on ok button)',
			                icon: "warning",
			                showCancelButton: false,
			                confirmButtonColor: '#3085d6',
		...
		function updateRemainingTime(){
	    		...
	    		$.ajax({
	    			type : 'POST',
	    			url : '/exam/updateRemainingTimeForLeads',
	    			data: JSON.stringify(body),
	                contentType: "application/json",
	                dataType : "json",
	                timeout : 10000,
...
function saveMedia(myFile) {
	try{
		...
		formData.append('image', myFileInput); 
		 $.ajax({
			url : '/exam/m/uploadTestAnswerFileForLeads?testId=${test.id}&userId=${userId}',
			data : formData,
			processData : false,
			contentType : false,
			type : 'POST',
