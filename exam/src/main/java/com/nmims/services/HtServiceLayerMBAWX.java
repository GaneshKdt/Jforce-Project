...
import com.nmims.helpers.HallTicketPDFCreatorMBAWX;

@Component
public class HtServiceLayerMBAWX {
	...
	@Autowired
	private MBAWXHallTicketDAO hallTicketDAO;

	@Autowired
	private HallTicketPDFCreatorMBAWX hallTicketCreator;
	...
	public MBAHallTicketBean createHallTicketDownload(String sapid,  HashMap<String, String> programMap) {
		...
		if("success".equals(hallTicketBean.getStatus())) {
			try {
				...
				hallTicketBean.setFileName(url);
	...
	private MBAHallTicketBean generateHallTicket(String sapid , HashMap<String, String> programMap) {
		...
		try {
			...
			MBAStudentDetailsBean studentDetails = studentDetailsDAO.getTimeboundDetailsForStudentForMonthYear(sapid, CURRENT_MBAWX_ACAD_MONTH, CURRENT_MBAWX_ACAD_YEAR);
