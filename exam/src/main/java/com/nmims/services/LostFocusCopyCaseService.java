...
@Service("lostFocusCopyCaseService")
public class LostFocusCopyCaseService {
	 ...
	 @Autowired
		...
		public void markCopyCaseBatchJob( TestExamBean test ){
			...
			try {
				...
				errorAnalytics = auditTrailsService.getErrorAnalyticsBySapidCreatedDate(errorformat.format( format.parse( test.getStartDate() ) ) );
			...
			try {
				...
				studentListForCopyCase = auditTrailsService.getLostFocusDetailsForBatchJob( bean ) ;
				...
				for( LostFocusLogExamBean logBean : studentListForCopyCase ) {
					...
					studentTestDetails = auditTrailsService.getStudentsTestDetailsBySapidAndTestId( logBean.getSapid(), bean.getTestId());
			...
			studentListForCopyCase = auditTrailsService.getLostFocusListWithStudentAndTestDetails( detailedStudentList );
		...
		private List<NetworkLogsExamBean> getNetworkLogsForSapid( AuditTrailExamBean bean ){
			...
			try {
				networkLogs = auditTrailsService.getNetworkLogsForSapid( bean );
			} catch (Exception e) {
		...
		private ArrayList<LostFocusLogExamBean> getIndividualStudentLostFocusLogs( AuditTrailExamBean bean ){
			...
			try {
				lostFocusLogs = auditTrailsService.getIndividualStudentLostFocusLogs( bean );
			} catch (Exception e) {
