...
public class UFMMarkScoredExcelView extends AbstractXlsxStreamingView  implements ApplicationContextAware{

	@Override
	protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		
		List<StudentMarksBean> list = (ArrayList<StudentMarksBean>) model.get("ufmMarkScoredRecords");
		//create a wordsheet :- START
		Sheet sheet = workbook.createSheet("UFM Mark Scored Records");
		...
		header.createCell(index++).setCellValue("Written Score");
		...
		for (int i = 0 ; i < list.size(); i++) {
			...
			row.createCell(index++).setCellValue(bean.getWrittenBeforeRIANV());
