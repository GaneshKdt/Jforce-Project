...
public class AssignmentUploadSuccessReportExcelView  extends AbstractXlsxStreamingView  implements ApplicationContextAware{
	
	@Override
	protected void buildExcelDocument(Map model, Workbook workbook,	HttpServletRequest request, HttpServletResponse response)
		throws Exception {
 
		List<AssignmentStatusBean> subjectMappingSucessBeanList = (List<AssignmentStatusBean>) model.get("subjectMappingSuccessBeanList");
		//create a wordsheet
		...
		int rowNum = 1;
		for (int i = 0 ; i < subjectMappingSucessBeanList.size(); i++) {
			//create the row data
			Row row = sheet.createRow(rowNum++);
			AssignmentStatusBean bean = subjectMappingSucessBeanList.get(i);
