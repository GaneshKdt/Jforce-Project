...
@Repository("mbaWxExamBookingDAO")
public class MBAWXExamBookingDAO extends BaseDAO{
	...
	public List<MBAStudentSubjectMarksDetailsBean> getFailedSubjectsForStudent(MBAStudentDetailsBean studentDetails) {
		...
		String sql = ""
			+ " SELECT "
				...
				+ " `tum`.`role` AS `role`, "
				+ " coalesce(`mbaebc`.`bookingAmount`, " + MBA_WX_EXAM_BOOKING_CHARGES +") AS `bookingAmount`, "
				+ " coalesce(`mbaebc`.`slotChangeAmount`, " + MBA_WX_EXAM_BOOKING_SLOT_CHANGE_CHARGES +") AS `slotChangeAmount` "
			+ " FROM "
			+ " `exam`.`mba_passfail` `mpf` "
			    ...
			    + " ON `tum`.`timebound_subject_config_id` = `mpf`.`timeboundId` "
			    	+ " AND `mpf`.`sapid` = `tum`.`userId` "
			    ...
			    /* get booking charges */
			    + " LEFT JOIN `exam`.`mba_wx_exam_booking_charges` `mbaebc` "
	...
	public List<MBAStudentSubjectMarksDetailsBean> getResitFailedSubjectsForStudent(MBAStudentDetailsBean studentDetails) {
		...
		List<MBAStudentSubjectMarksDetailsBean> list = jdbcTemplate.query(
			sql, 
			new Object[]{ 
				studentDetails.getSapid(), CURRENT_MBAWX_ACAD_YEAR, CURRENT_MBAWX_ACAD_MONTH
			},
			new BeanPropertyRowMapper<MBAStudentSubjectMarksDetailsBean>(MBAStudentSubjectMarksDetailsBean.class)
		);
	...
	@Transactional(readOnly = true)
	public List<MBAStudentSubjectMarksDetailsBean> getResitFailedSubjectsForStudent(String sapid, String acadYear, String acadMonth) {
		jdbcTemplate = new JdbcTemplate(dataSource);
		//Get list of Resit subjects for current cycle (if any)
		String sql = ""
				+ " SELECT "
	...
	public boolean checkIfExamBookingLive(MBAStudentDetailsBean mbaStudentDetailsBean) {
 		...
 		int count = (int) jdbcTemplate.queryForObject(
 			...
 			new Object[]{ 
				...
				mbaStudentDetailsBean.getConsumerProgramStructureId()
 			},
 			Integer.class
 		);
	...
	public List<MBACentersBean> getAllCenters() {
		...
		String sql = ""
				+ "SELECT * FROM `exam`.`mba_wx_centers` WHERE `active` = 'Y'";
	...
	public List<MBAExamBookingRequest> getAllStudentApprovedBookings(MBAStudentDetailsBean studentDetails) {
		...
		String sql = ""
				...
				+ " FROM `exam`.`mba_wx_payment_records` `pr` "
				
				+ " INNER JOIN `exam`.`mba_wx_bookings` `b` "
	...
	public List<String> getSuccessfulBookingsForSeatRelease(MBAExamBookingRequest bookingRequest) {
		...
		String sql = ""
				+ " SELECT `bookingId` FROM `exam`.`mba_wx_bookings` WHERE `timeboundId` IN ( "
					+ " SELECT `timeboundId` "
					+ " FROM `exam`.`mba_wx_bookings` `b` "
	...
	public ArrayList<MBAExamBookingRequest> getUnSuccessfulExamBookings(String sapid) {
		jdbcTemplate = new JdbcTemplate(dataSource);
		String sql = "SELECT mwxb.bookingId,mwxb.sapid,mwxb.slotId,mwxb.term,mwxb.year,mwxb.month,mwxb.paymentRecordId,mwxb.bookingStatus," +
				"mwxp.paymentOption,mwxp.trackId,mwxp.amount,mwxp.tranStatus " +
				"FROM exam.mba_wx_bookings mwxb, exam.mba_wx_payment_records mwxp where mwxb.paymentRecordId= mwxp.id " +
				"and mwxb.sapid = ?  " +
				"and mwxb.bookingStatus <> 'Y' " +
				"and mwxb.bookingStatus <> 'RL' " +
				"and mwxb.bookingStatus <> 'RF' " +
				"and mwxb.bookingStatus <> 'Online Payment Manually Approved' " +
				"group by mwxp.trackId";
	...
	public boolean checkIfStudentHasSuccessfulBookingsForTimeboundId(String timeboundId, String sapid) {
		...
		String sql = ""
				+ " SELECT "
					+ " count(*) "
				+ " FROM `exam`.`mba_wx_bookings` `b` " 
				+ " LEFT JOIN `exam`.`mba_wx_payment_records` `pr` " 
					+ " ON `pr`.`id` = `b`.`paymentRecordId` " 
	...
	public ArrayList<MBAExamBookingRequest> getConfirmedBooking(String sapid) {
		jdbcTemplate = new JdbcTemplate(dataSource);
		String sql = "SELECT mwxb.bookingId,mwxb.sapid,mwxb.slotId,mwxb.term,mwxb.year,mwxb.month,mwxb.paymentRecordId,mwxb.bookingStatus, " 
				+ " mwxp.paymentOption,mwxp.trackId,mwxp.amount,mwxp.amount,mwxp.tranStatus " 
				+ " FROM exam.mba_wx_bookings mwxb, exam.mba_wx_payment_records mwxp where mwxb.paymentRecordId= mwxp.id "
				+ " and mwxb.sapid = ? "
				+ " and bookingStatus = 'Y' "
	...
	public void addConflictTransaction(MBAExamBookingRequest conflictRequest) {
		...
		final String sql = " "
			+ " INSERT INTO `exam`.`mba_wx_exam_booking_conflict_transactions` "
			+ " ( "
				+ " `trackId`, `bookingId`, "
	...
	public List<MBAExamConflictTransactionBean> getAllConflictTransactions(String year,String month) {
		...
		String sql = " "
				+ " SELECT * "
					+ " FROM `exam`.`mba_wx_exam_booking_conflict_transactions` `c` "
				
				+ " RIGHT JOIN `exam`.`mba_wx_bookings` `b` "
