...
@Repository("mbawxTimeTableDAO")
public class MBAWXTimeTableDAO extends BaseDAO {
	...
	public String deleteTimeTableById(String timeTableId){
		try {
			String sql = "delete from exam.mba_wx_time_table where timeTableId=?";
			jdbcTemplate = new JdbcTemplate(dataSource);
	...
	public List<MBATimeTableBean> batchInsertTimeTables(final List<MBATimeTableBean> timeTableBeansList) {
		...
		String sql = ""
				+ " INSERT INTO `exam`.`mba_wx_time_table` "
				+ " ( "
	...
	public Long getProgramSemSubjectId(MBATimeTableBean bean) {
		try {
			List<Object> queryParam = new ArrayList<Object>(Arrays.asList(bean.getTerm(), bean.getSubjectName(), bean.getExamMonth(), bean.getExamYear()));
			String sql = ""
					+ " SELECT "
					...
					if("fifteen months".equals(bean.getProgram())) {
						sql = sql+ " AND cps.id in (111,151) ";
					}
					else if ("twenty four months".equals(bean.getProgram())) {
						sql = sql+ " AND cps.id =160 ";
					}
					else {
						sql = sql+ " AND `p`.`code` = ? ";
						queryParam.add(bean.getProgram());
					}
					
					sql = sql+ " GROUP BY `pss`.`id` ";
