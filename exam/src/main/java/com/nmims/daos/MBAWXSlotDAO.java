...
@Repository("mbawxSlotDAO")
public class MBAWXSlotDAO extends BaseDAO {
	...
	public List<MBASlotBean> getSlotsListByTimeTableId(String slotId){
		...
		jdbcTemplate = new JdbcTemplate(dataSource);
		List<MBASlotBean> mbawxCentersBeansList = jdbcTemplate.query(
			sql,
			new Object[] { slotId },
			new BeanPropertyRowMapper<MBASlotBean>(MBASlotBean.class)
		);
		return mbawxCentersBeansList;
	}
	...
	public String deleteSlotById(String slotId){
		try {
			String sql = "DELETE FROM `exam`.`mba_wx_slots` WHERE `slotId` = ?";
			jdbcTemplate = new JdbcTemplate(dataSource);
