...
public class MBAWXTranscriptPDFCreator {
	...
	public String createTrascript(MBATranscriptBean transcriptBean, String HALLTICKET_PATH, Map<String, ProgramExamBean> programDetailMap) throws Exception{
		...
		try{
			...
			if(MBAWX_JUL22_MASTERKEY.equals(transcriptBean.getStudent().getConsumerProgramStructureId()))
			{
				document.newPage();
				document.add(emptyPara);
				addPageHeader(document, transcriptBean);
	...
	private void generateStudentInfoTable(Document document, MBATranscriptBean bean, PdfWriter writer, Map<String, ProgramExamBean> programDetailMap) throws Exception {
		try {
			...
			addStudentInfoTableCells("Program Name / Instruction Medium: ", "Master of Business Administration (Working Executives) / English", studentInfoTable);
...
//			subjectsTable.setSpacingAfter(5f);
	...
	private void addTrimesterPatternInfoTable(Document document, String consumerProgramStructureId) throws DocumentException {
		...
		if("111".equals(consumerProgramStructureId)) {
		...
		}else if("151".equals(consumerProgramStructureId)) {
			addTrimesterPatternTableData("Academic Learning Activities", "1 credit", "30 hrs", trimesterPatternTable);
			addTrimesterPatternTableData("Capstone Project", "12 credits", "220 hrs (for 6 weeks)", trimesterPatternTable);
		}else if(MBAWX_JUL22_MASTERKEY.equals(consumerProgramStructureId)) {
			addTrimesterPatternTableData("Academic Learning Activities", "1 credit", "30 hrs", trimesterPatternTable);
			addTrimesterPatternTableData("Capstone Project", "8 credits", "220 hrs (for 6 weeks)", trimesterPatternTable);
		}
