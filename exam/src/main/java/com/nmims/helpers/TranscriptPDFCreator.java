...
public class TranscriptPDFCreator {
	...
	private final static String CREDIT_ENROLL_DATE = "01-Jul-2021"; //applicable date to get credit column in transcript
	private final static List<String> MBA_DISTANCE_MASTERKEYS = Arrays.asList("132","133","134","135","136","137","138","139","140","141","161");
	...
	public String createTrascript(ArrayList<PassFailExamBean> passList, StudentExamBean student, List<StudentExamBean> lateralStudentList, HashMap<String, ProgramExamBean> programCodeNameMap, 
			...
			Map<String, ProgramExamBean> programDetailMap,StudentMarksDAO dao, boolean isSoftcopy,HashMap<String, String> lateralSapidLastExamMonthYearMap,
			Map<String, MDMSubjectCodeMappingBean> mapPssDetail, Map<Integer, MDMSubjectCodeMappingBean> mapSubjectCredit) throws Exception{
...
//			document.setMargins(30, 30, 100, 25); //Left,right,top,bottom//
			...
			boolean isApplicableForCredits = false;
			try {
				//To validate credit is applicable or not
				isApplicableForCredits = checkCreditsInTranscriptApplicable(student.getEnrollmentYear(),student.getEnrollmentMonth(),student.getConsumerProgramStructureId());
			}
			catch(Exception e)
			{
				//e.printStackTrace();
				throw new Exception("Error in validating student enrollment date: "+e);
			}
			...
			generateStudentInfoTable(passList, document, student, lateralStudentList, programCodeNameMap, writer, lastExamMonth,
					programDetailMap.get(student.getProgram()+"-"+student.getPrgmStructApplicable()).getProgramDuration(),
					programDetailMap.get(student.getProgram()+"-"+student.getPrgmStructApplicable()).getProgramDurationUnit(),dao,lateralSapidLastExamMonthYearMap);
			...
			/*List<String> programTypes =new ArrayList(Arrays.asList("Professional Diploma","Diploma Programs","Diploma","Certificate Programs","Certificate","Modular Program","diploma in finance","Professional Certificate"));
 			
			if(programTypes.contains(programType)) {
			//if(student.getProgram().startsWith("D") || student.getProgram().startsWith("PD") || student.getProgram().startsWith("C") || student.getProgram().startsWith("A")){
			...
			List<String> pgProgramTypes =new ArrayList(Arrays.asList("Post Graduate Diploma","Master","Executive Programs","MBA (Distance)","Bachelor Programs","Degree Program"));
 			
			if(pgProgramTypes.contains(programType)) {
			//if(student.getProgram().startsWith("PG")){
				for(int i = 1; i <= noOfSemesters-2 ;i++){
				generateSemWiseMarksTable(document, passList, student,i, examOrderMap,semWiseSubjectMap,dao);
				}
				...
				Paragraph contd = new Paragraph("....contd Page 2" , font8);
				contd.setAlignment(Element.ALIGN_RIGHT);
				document.add(contd);
				...
				generateStudentInfoTable(document, student, programCodeNameMap, writer, lastExamMonth,
						programDetailMap.get(student.getProgram()+"-"+student.getPrgmStructApplicable()).getProgramDuration(),
						programDetailMap.get(student.getProgram()+"-"+student.getPrgmStructApplicable()).getProgramDurationUnit());
				
				for(int i = 1; i <= noOfSemesters-2 ;i++){
				generateSemWiseMarksTable(document, passList, student, i+2, examOrderMap,semWiseSubjectMap,dao);
				}
				generateFinalPercentage(document);
			...
			//New logic for sem 1 to 8
			//For sem 1 and 2only
			if(noOfSemesters==1 || noOfSemesters==2){
				for(int i = 1; i <= noOfSemesters ;i++){
					if(isApplicableForCredits)
						generateSemWiseMarksTableWithCredit(document, passList, student, i, examOrderMap,semWiseSubjectMap,dao,mapSubjectCredit,mapPssDetail);
					else
						generateSemWiseMarksTable(document, passList, student, i, examOrderMap,semWiseSubjectMap,dao);
				}
				generateFinalPercentage(document,isApplicableForCredits);
			
				if(isSoftcopy && !"Y".equalsIgnoreCase(student.getFromAdmin())) {
					generateWatermarkAndCOESign(document, writer);
				}
				generateCOESign(document);
...
//For sem 1 to 4
			if(noOfSemesters>=3){
				...
				if(noOfSemesters==3 || noOfSemesters==4){
					for(int i = 3; i <= noOfSemesters ;i++){
						if(isApplicableForCredits)
							generateSemWiseMarksTableWithCredit(document, passList, student, i, examOrderMap,semWiseSubjectMap,dao,mapSubjectCredit,mapPssDetail);
						else
							generateSemWiseMarksTable(document, passList, student, i, examOrderMap,semWiseSubjectMap,dao);
...
//For sem 1 to 6				
			if(noOfSemesters>=5){
				...
				Paragraph contd = new Paragraph("....contd Page 3" , font8);
				contd.setAlignment(Element.ALIGN_RIGHT);
				document.add(contd);
				...
				if(noOfSemesters==5 || noOfSemesters==6){
					for(int i = 5; i <= noOfSemesters ;i++){
						if(isApplicableForCredits)
							generateSemWiseMarksTableWithCredit(document, passList, student, i, examOrderMap,semWiseSubjectMap,dao,mapSubjectCredit,mapPssDetail);
						else
							generateSemWiseMarksTable(document, passList, student, i, examOrderMap,semWiseSubjectMap,dao);
...
//For sem 1 to 8	
			...
			if(noOfSemesters>=7){
				...
				Paragraph contd = new Paragraph("....contd Page 4" , font8);
				contd.setAlignment(Element.ALIGN_RIGHT);
				document.add(contd);
				...
				if(noOfSemesters==7 || noOfSemesters==8){
					for(int i = 7; i <= noOfSemesters ;i++){
						if(isApplicableForCredits)
							generateSemWiseMarksTableWithCredit(document, passList, student, i, examOrderMap,semWiseSubjectMap,dao,mapSubjectCredit,mapPssDetail);
						else
							generateSemWiseMarksTable(document, passList, student, i, examOrderMap,semWiseSubjectMap,dao);
			...
			if (!"Y".equalsIgnoreCase(student.getFromAdmin())) {
				Paragraph note = new Paragraph("Note:", font4);
				note.setAlignment(Element.ALIGN_BOTTOM);
				note.setSpacingBefore(10f);
				Paragraph disclaimer = new Paragraph("The above result is provisional in nature and is just for student's information. The transcript issued by the University will be the authentic document.",font6);
				disclaimer.setAlignment(Element.ALIGN_BOTTOM);
				document.add(note);
			...
			/*
			 * 	//Commented by Steffi on 8/10/2018
			 * 
			 * if(student.getProgram().startsWith("PG")){
				generateSemWiseMarksTable(document, passList, student, "1", examOrderMap,semWiseSubjectMap);
				generateSemWiseMarksTable(document, passList, student, "2", examOrderMap,semWiseSubjectMap);
				...
				generateSemWiseMarksTable(document, passList, student, "3", examOrderMap,semWiseSubjectMap);
				generateSemWiseMarksTable(document, passList, student, "4", examOrderMap,semWiseSubjectMap);
			...
			if(student.getProgram().startsWith("D")){
				generateSemWiseMarksTable(document, passList, student, "1", examOrderMap,semWiseSubjectMap);
				generateSemWiseMarksTable(document, passList, student, "2", examOrderMap,semWiseSubjectMap);
				generateFinalPercentage(document);
				//document.add( Chunk.NEWLINE );
			...
			if(student.getProgram().startsWith("C")){
				generateSemWiseMarksTable(document, passList, student, "1", examOrderMap,semWiseSubjectMap);
				generateFinalPercentage(document);
				//document.add( Chunk.NEWLINE );
				//document.add( Chunk.NEWLINE );
			 ...
			 //Added by Steffi
			  if(student.getProgram().startsWith("A")){
				generateSemWiseMarksTable(document, passList, student, "1", examOrderMap,semWiseSubjectMap);
				generateSemWiseMarksTable(document, passList, student, "2", examOrderMap,semWiseSubjectMap);
				generateFinalPercentage(document);
				generateCOESign(document);
	...
	public boolean checkCreditsInTranscriptApplicable(String enrollmentYear, String enrollmentMonth, String consumerProgramStructureId)throws Exception
	{
		String studentEnrollDate = "1-"+enrollmentMonth+"-"+enrollmentYear;
		SimpleDateFormat formatter = new SimpleDateFormat("dd-MMM-yyyy");
		Date enrollDate = formatter.parse(studentEnrollDate);
		Date creditEnrollDate = formatter.parse(CREDIT_ENROLL_DATE);
		if(MBA_DISTANCE_MASTERKEYS.contains(consumerProgramStructureId) && (creditEnrollDate.equals(enrollDate) || creditEnrollDate.before(enrollDate)))
			return true;
		else
			return false;
	}
...
//		PdfPTable coeTable = new PdfPTable(1);
//		coeTable.getDefaultCell().setBorder(0);
...
//		coeTable.addCell(coeCell);
//		coeTable.completeRow();
//
//		coeTable.setWidthPercentage(100);
//		coeTable.setSpacingBefore(30f);
//		coeTable.setSpacingAfter(15f);
//		document.add(coeTable);
	}
...
//	}
	...
	private void generateFinalPercentage(Document document, boolean isApplicableForCredits) throws DocumentException {
		
		PdfPTable remarksPara = new PdfPTable(2); // 2 columns.
		remarksPara.getDefaultCell().setBorder(0);
		
		double percentage = (double)(marksObtained * 100 / totalMarks);
		percentage = Math.round(percentage * 100.0) / 100.0;
		
		PdfPCell percentageCol = new PdfPCell(new Paragraph("Percentage Obtained: " + percentage , font9));
		PdfPCell totalCreditsCol = new PdfPCell(new Paragraph("Total Credits: " + totalCredits , font9));
		
		percentageCol.setBorder(0);
		totalCreditsCol.setBorder(0);
		remarksPara.addCell(percentageCol);
		
		if(isApplicableForCredits)
			remarksPara.addCell(totalCreditsCol);
		
		remarksPara.completeRow();

		remarksPara.setWidthPercentage(100);
		remarksPara.setSpacingBefore(5f);
		float[] columnWidths = {1.2f, 3.5f};
		remarksPara.setWidths(columnWidths);
	...
	private void generateCOETable(Document document) throws DocumentException {
		...
		/*Paragraph lable1 = new Paragraph("MU" , font8);
		lable1.setAlignment(Element.ALIGN_LEFT);
		document.add(lable1);*/
		
		/*Paragraph label2 = new Paragraph("KM/- " , font8);
		label2.setAlignment(Element.ALIGN_LEFT);
		document.add(label2);*/
	}
	...
	private void generateCOESign(Document document) throws Exception {
		try {
			...
			PdfPCell coeCell = new PdfPCell(new Paragraph("Controller of Examinations", font8));
	...
	/*private String generateStudentPassword(StudentBean student, ArrayList<ExamBookingTransactionBean> subjectsBooked) throws Exception {
	...
	/*private void generateInstructions(Document document, StudentBean student, String mostRecentTimetablePeriod) throws Exception {
		try {
			...
			orderedList.add(new ListItem("It is Mandatory for the student to carry Student ID Card and Hall Ticket for the examination. Without both the documents student will strictly not be allowed in the Examination Hall.", font11));
			orderedList.add(new ListItem("This Hall Ticket is valid for the subject/s mentioned in the Hall Ticket for the above mentioned dates and Exam Center displayed.", font6));
			
			if("Jul2014".equals(programStructure)){
				orderedList.add(new ListItem("It is Mandatory for the student to first register at the Registration Desk for each exam. Without Registration student will not be allowed to appear for the exam. Students are expected to report atleast one hour before actual exam time.", font11));
			...
			}else if("Jul2014".equals(programStructure)){
				orderedList.add(new ListItem(" Exam will start and end as per the scheduled time mentioned in the hall ticket. Exam duration is 2 hours. Students reaching after the start of exam will not be permitted extra exam time. Students will not be allowed entry 30 minutes after the start of the Exam.", font11));
			}else{
			...
			if("Jul2014".equals(programStructure)){
				orderedList.add(new ListItem("Only standard 12 digit calculators will be allowed for the Term End Examination. "
						...
						+ "Students will not be allowed to borrow calculators during the examination. Programmable / Scientific calculators "
						+ "are strictly prohibited. Non adherence will result in examination getting Null and Void.", font6));
				
			}else{
				orderedList.add(new ListItem("Only standard 12 digit calculators will be allowed for the Term End Examination. "
						+ "It is mandatory for the students to carry their calculators. Students will not be allowed to borrow calculators during the examination. "
	...
	private void generateStudentInfoTable(ArrayList<PassFailExamBean> passList, Document document, StudentExamBean student, List<StudentExamBean> lateralStudentList,HashMap<String, ProgramExamBean> programCodeNameMap, 
			 PdfWriter writer, String lastExamMonthYear,String duration, String durationUnits,StudentMarksDAO dao,HashMap<String, String> lateralSapidLastExamMonthYearMap) throws Exception {
		try {
			...
			studentInfoTable.completeRow();
			//Added `modeOfLearning` on the behalf of `Distance Learning Mode` 
			String programDuration = StringUtils.capitalize(duration)+" "+StringUtils.capitalize(durationUnits)+" ("+programDetails.getModeOfLearning()+")";
			/*
			 * Commented by Steffi getting programDuration from database
			 * String programDuration = "";
			 * if(student.getProgram().startsWith("PG")){
				programDuration = "Two Years (Distance Learning Mode)";
			}else if(student.getProgram().startsWith("D")){
				programDuration = "One Year (Distance Learning Mode)";
			}else if(student.getProgram().startsWith("C")){
				programDuration = "Six Months (Distance Learning Mode)";
			}*/
			
			
			PdfPCell programDurationLabelCell = new PdfPCell(new Paragraph("Duration of the Program (Mode of Delivery):", font11));
...
//			Commented by Somesh display Month & Year of last exam cleared in pdf
//			lastExamMonth=student.getValidityEndMonth();
			//lastExamMonthYear = lastExamMonthYear.substring(0, 3);
			//lastExamMonthYear = lastExamMonthYear.replaceAll("Apr", "April");
			//lastExamMonthYear = lastExamMonthYear.replaceAll("Jun", "June");
			//lastExamMonthYear = lastExamMonthYear.replaceAll("Sep", "September");
			//lastExamMonthYear = lastExamMonthYear.replaceAll("Dec", "December");
			
			//Adding previous program in student table info
			...
			if(student.getIsLateral().equalsIgnoreCase("Y") && student.getPreviousStudentId() != null) {
				...
				for(StudentExamBean lateralStudent :lateralStudentList) {
					String lateralEnrollmentMonth = lateralStudent.getEnrollmentMonth();
					String lateralLastExamMonthYear = "";
					if(lateralSapidLastExamMonthYearMap.containsKey(lateralStudent.getSapid())){
						lateralLastExamMonthYear = lateralSapidLastExamMonthYearMap.get(lateralStudent.getSapid());
					}
					//lateralEnrollmentMonth = lateralEnrollmentMonth.substring(0, 3);
					//lateralLastExamMonthYear = lateralLastExamMonthYear.substring(0, 3);
					//lateralEnrollmentMonth = lateralEnrollmentMonth.replaceAll("Jul", "July");
					//lateralEnrollmentMonth = lateralEnrollmentMonth.replaceAll("Jan", "January");
					//lateralLastExamMonthYear = lateralLastExamMonthYear.replaceAll("Apr", "April");
					///lateralLastExamMonthYear = lateralLastExamMonthYear.replaceAll("Jun", "June");
					//lateralLastExamMonthYear = lateralLastExamMonthYear.replaceAll("Sep", "September");
					//lateralLastExamMonthYear = lateralLastExamMonthYear.replaceAll("Dec", "December");
				
					String key1 = lateralStudent.getProgram() + "-" + lateralStudent.getPrgmStructApplicable();
					ProgramExamBean lateralprogramDetails = programCodeNameMap.get(key1);
					String lateralprogramFullName = lateralprogramDetails.getProgramname();
					if("PGDMM-MLI".equals(lateralStudent.getProgram())){
						lateralprogramFullName = lateralprogramFullName + " � MLI (Max Life Insurance) ";
					}
					chunk = new Chunk(lateralprogramFullName+"/English"+" "+"-"+" "+lateralEnrollmentMonth+"-"+lateralStudent.getEnrollmentYear()+"/"+lateralLastExamMonthYear+"\n", font12);
					datePhrase.add(chunk);// to get all lateral programs detail in header
					//datePhrase = new Phrase(new Chunk(lateralprogramFullName+"/English"+" "+"-"+" "+lateralEnrollmentMonth+"-"+lateralData.getEnrollmentYear()+"/"+lateralLastExamMonthYear+"\n", font12));
				}
				datePhrase.add(new Phrase(new Chunk(student.getProgram()+" "+"(Lateral Entry)"+" "+"-"+" "+ enrollmentMonth+"-"+student.getEnrollmentYear()+"/"+lastExamMonthYear, font12)));					
				enrollmentValueCell = new PdfPCell(datePhrase);
			}else {
	...
	private void generateSemWiseMarksTable(Document document, ArrayList<PassFailExamBean> passList, StudentExamBean student, int semester, HashMap<String, BigDecimal> examOrderMap,HashMap<String,ArrayList<String>> semWiseSubjectMap,StudentMarksDAO dao) throws Exception {
		...
		try{
			...
			if(student.getWaivedOffSubjects() !=null){
				if(semWiseSubjectMap.containsKey(sem)){
				for(String subject : semWiseSubjectMap.get(sem)){
					if(student.getWaivedOffSubjects().contains(subject.trim())){
						numberOfWaivedOffSubject ++;
					}
				}
			}
			}
			...
			//Added by Steffi for showing lateral students as passed if all sem subjects are waived off.
			if(noOfSubjectsPassed == 0 && numberOfApplicableSubjectForSem != numberOfWaivedOffSubject){
			...
			/*else if(noOfSubjectsPassed == 0 && numberOfApplicableSubjectForSem == numberOfWaivedOffSubject){
				Paragraph noticePara = new Paragraph("The student has passed the examination of Semester " + sem, font6);
				semesterNamePara.setAlignment(Element.ALIGN_LEFT);
				document.add(noticePara);
				return;
			}*/
			...
			for (int j = 0; j < passList.size(); j++) {
				...
				if(bean.getTotal().isEmpty() || StringUtils.isBlank(bean.getTotal())){
					...
					marksObtained += Integer.parseInt(bean.getTotal());
				}
				totalMarks += 100.0;
				...
				/*
				 * Commented by Sanket 10-Jun-2017 for considering latest Exam Month-Year of Written or Assignment
				 *  
				 * String examMonth = bean.getWrittenMonth().replaceAll("Apr", "April");
				examMonth = examMonth.replaceAll("Jun", "June");
				examMonth = examMonth.replaceAll("Sep", "September");
				examMonth = examMonth.replaceAll("Dec", "December");*/
				
				String examMonthYear = getLatestExamMonthYear(bean, examOrderMap);
				System.out.println("getLatestExamMonthYear"+ examMonthYear.toString());
	...
	private void generateSemWiseMarksTableWithCredit(Document document, ArrayList<PassFailExamBean> passList, 
			StudentExamBean student, int semester, HashMap<String, BigDecimal> examOrderMap,
			HashMap<String,ArrayList<String>> semWiseSubjectMap,StudentMarksDAO dao,
			Map<Integer, MDMSubjectCodeMappingBean> mapSubjectCredit, Map<String, MDMSubjectCodeMappingBean> mapPssDetail) throws Exception {
		...
		try{
			...
			PdfPTable subjectsTable = new PdfPTable(7);

			PdfPCell srNoCell = new PdfPCell(new Paragraph("Sr. No", font4));
			PdfPCell subjectsCell = new PdfPCell(new Paragraph("Subject", font4));
			PdfPCell creditsCell = new PdfPCell(new Paragraph("Credit", font4));
			PdfPCell monthYearCell = new PdfPCell(new Paragraph("Month & Year of Examination", font4));
			PdfPCell maxMarksCell = new PdfPCell(new Paragraph("Maximum Marks", font4));
			PdfPCell minMarksCell = new PdfPCell(new Paragraph("Minimum Marks", font4));
			PdfPCell totalMarksCell = new PdfPCell(new Paragraph("Marks Obtained", font4));
			...
			subjectsTable.addCell(srNoCell);
			subjectsTable.addCell(subjectsCell);
			subjectsTable.addCell(creditsCell);
			subjectsTable.addCell(monthYearCell);
			subjectsTable.addCell(maxMarksCell);
			subjectsTable.addCell(minMarksCell);
			subjectsTable.addCell(totalMarksCell);
			...
			for (int j = 0; j < passList.size(); j++) {
				...
				//Extract subjectCredits by masterKey, subject and sem for each subjects
				String key = student.getConsumerProgramStructureId()+"-"+bean.getSubject()+"-"+sem.toString();
				String credits = "NA";
				if(mapPssDetail.containsKey(key))
				{
					MDMSubjectCodeMappingBean extractedPssDetail = mapPssDetail.get(key);
					if(mapSubjectCredit.containsKey(extractedPssDetail.getId()))
					{
						credits = mapSubjectCredit.get(extractedPssDetail.getId()).getSubjectCredits().toString();
					}
				}
				...
				if(!"NA".equalsIgnoreCase(credits))
					totalCredits += Double.parseDouble(credits);
				
				srNoCell = new PdfPCell(new Paragraph(count+"", font6));
				subjectsCell = new PdfPCell(new Paragraph(bean.getSubject(), font6));
				
				creditsCell = new PdfPCell(new Paragraph(credits, font6));
				String examMonthYear = getLatestExamMonthYear(bean, examOrderMap);
				monthYearCell = new PdfPCell(new Paragraph(examMonthYear, font6));
				...
				srNoCell.setFixedHeight(18f);
				srNoCell.setHorizontalAlignment(Element.ALIGN_CENTER); srNoCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
				creditsCell.setHorizontalAlignment(Element.ALIGN_CENTER); creditsCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
				monthYearCell.setHorizontalAlignment(Element.ALIGN_CENTER); monthYearCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
				maxMarksCell.setHorizontalAlignment(Element.ALIGN_CENTER); maxMarksCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
				minMarksCell.setHorizontalAlignment(Element.ALIGN_CENTER); minMarksCell.setVerticalAlignment(Element.ALIGN_MIDDLE);
				totalMarksCell.setHorizontalAlignment(Element.ALIGN_CENTER);  totalMarksCell.setVerticalAlignment(Element.ALIGN_MIDDLE); 
			...
			subjectsTable.setSpacingBefore(5f);
			subjectsTable.setSpacingAfter(5f);
			float[] marksColumnWidths = {1f, 6f, 2.5f, 5f, 2.5f, 2.5f,2.5f};
			subjectsTable.setWidths(marksColumnWidths);
			document.add(subjectsTable);
			Paragraph remarksPara = new Paragraph("Remarks: " + getPassFailStatus(student, numberOfSubjectsPassed, sem,numberOfWaivedOffSubject, dao) , font9);
			remarksPara.setAlignment(Element.ALIGN_LEFT);
	...
	private String getLatestExamMonthYear(PassFailExamBean bean, HashMap<String, BigDecimal> examOrderMap) {
		String writtenExamYearMonth = bean.getWrittenMonth()+ bean.getWrittenYear();
		String assignmentYearMonth = bean.getAssignmentMonth() + bean.getAssignmentYear();
		String examMonthYearToConsiderForTranscript = "";
		
		if("Project".equals(bean.getSubject())){
			examMonthYearToConsiderForTranscript =  bean.getWrittenMonth()+ "-" + bean.getWrittenYear();
		}else if("Module 4 - Project".equals(bean.getSubject())){
			examMonthYearToConsiderForTranscript =  bean.getWrittenMonth()+ "-" + bean.getWrittenYear();
		}else if(writtenExamYearMonth == null || "".equals(writtenExamYearMonth)){
			examMonthYearToConsiderForTranscript = bean.getAssignmentMonth() + "-" + bean.getAssignmentYear();
		}else if(assignmentYearMonth == null || "".equals(assignmentYearMonth)){
			examMonthYearToConsiderForTranscript = bean.getWrittenMonth()+ "-" + bean.getWrittenYear();;
		}else if(examOrderMap.get(writtenExamYearMonth).floatValue() >= examOrderMap.get(assignmentYearMonth).floatValue()){
			examMonthYearToConsiderForTranscript = bean.getWrittenMonth()+ "-" + bean.getWrittenYear();
		}else{
			examMonthYearToConsiderForTranscript = bean.getAssignmentMonth() + "-" + bean.getAssignmentYear();
		}
	...
	private String getPassFailStatus(StudentExamBean student,int numberOfSubjectsPassed,String sem,int numberOfWaivedOffSubject,StudentMarksDAO dao) {
		...
		if((student.getProgram().equalsIgnoreCase("ACDM")&&sem.equalsIgnoreCase("2")) || (student.getProgram().equalsIgnoreCase("ACOM")&&sem.equalsIgnoreCase("2"))){
			noOfApplicableSubjectsToClearSem=noOfApplicableSubjectsToClearSem - 1;
		}
		
	/*Commented on 8-28-2018 by Steffi
	 * 
	 * 	if("Online".equals(student.getExamMode())){
			//Online students
			if(program.startsWith("P")){
				noOfSubjectsToClearSem = 6;
				noOfApplicableSubjectsToClearSem = 6;
			}else if(program.startsWith("D")){
				noOfSubjectsToClearSem = 6;
				noOfApplicableSubjectsToClearSem = 6;
			}else if(program.startsWith("C")){
				noOfSubjectsToClearSem = 5;
				noOfApplicableSubjectsToClearSem = 5;
			}
		}else{
			//Offline students
			if(program.startsWith("P")){
		...
		//noOfApplicableSubjectsToClearSem = noOfApplicableSubjectsToClearSem - numberOfWaivedOffSubject;
		//end
		
		//totalPassedSubjects = numberOfSubjectsPassed ; // for lateral students considering pass
...
//			
//			Paragraph datePara = new Paragraph("Date: "+markSheetDate, font6);
//			datePara.setAlignment(Element.ALIGN_RIGHT);
//			document.add(datePara);
	...
	private void generateHeaderTableWithoutLogo(Document document, StudentExamBean student) throws Exception {
		...
		try {
//			logo.scaleAbsolute(142,40);
			//logo = Image.getInstance(new URL("https://studentzone-ngasce.nmims.edu/exam/resources_2015/images/nmims_logo_new.jpg"));
			//Image signature = Image.getInstance(new URL("http://admissions-ngasce.nmims.edu:4001/StudentDocuments/studentPhotos/signature.jpg"));
