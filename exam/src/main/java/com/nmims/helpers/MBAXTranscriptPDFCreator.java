...
public class MBAXTranscriptPDFCreator {
	...
	public String createTrascript(MBATranscriptBean transcriptBean, String HALLTICKET_PATH, Map<String, ProgramExamBean> programDetailMap) throws Exception{
		...
		try{
			...
			int pageNum = 1;
			addPageFooter(document, writer, pageNum++, transcriptBean.isSoftCopy(),transcriptBean.getLogoRequired(),transcriptBean.getStudent().getConsumerProgramStructureId());

			document.newPage();
			document.add(emptyPara);
			addPageHeader(document, transcriptBean);
			generateSemWiseMarksTable(document, transcriptBean, 2);
			generateSemWiseMarksTable(document, transcriptBean, 3);
			addPageFooter(document, writer, pageNum++, transcriptBean.isSoftCopy(),transcriptBean.getLogoRequired(),transcriptBean.getStudent().getConsumerProgramStructureId());
			
			document.newPage();
			document.add(emptyPara);
			addPageHeader(document, transcriptBean);
			generateSemWiseMarksTable(document, transcriptBean, 4);
			generateSemWiseMarksTable(document, transcriptBean, 5);
			addPageFooter(document, writer, pageNum++, transcriptBean.isSoftCopy(),transcriptBean.getLogoRequired(),transcriptBean.getStudent().getConsumerProgramStructureId());

			if(transcriptBean.getSemSubjectList().size()==6) {
				document.newPage();
				document.add(emptyPara);
				addPageHeader(document, transcriptBean);
				generateSemWiseMarksTable(document, transcriptBean, 6);
				addPageFooter(document, writer, pageNum++, transcriptBean.isSoftCopy(),transcriptBean.getLogoRequired(),transcriptBean.getStudent().getConsumerProgramStructureId());
			}
			
			if(transcriptBean.getSemSubjectList().size()==8) {
				document.newPage();
				document.add(emptyPara);
				addPageHeader(document, transcriptBean);
				generateSemWiseMarksTable(document, transcriptBean, 6);
				generateSemWiseMarksTable(document, transcriptBean, 7);
				generateSemWiseMarksTable(document, transcriptBean, 8);
				addPageFooter(document, writer, pageNum++, transcriptBean.isSoftCopy(),transcriptBean.getLogoRequired(),transcriptBean.getStudent().getConsumerProgramStructureId());
			}
			...
			addExplainatoryDetailsPage(document);
			addPageFooter(document, writer, pageNum++, transcriptBean.isSoftCopy(),transcriptBean.getLogoRequired(),transcriptBean.getStudent().getConsumerProgramStructureId());
	...
	private void addPageFooter(Document document, PdfWriter writer, int pageNum, boolean isSoftCopy,String logoRequired, String CPSID) throws MalformedURLException, IOException, DocumentException {
		...
		if(CPSID.equalsIgnoreCase("119")) {
			pageNumCell = new PdfPCell(new Paragraph("Page " + pageNum + " of 4", font10));
		}else {
			pageNumCell = new PdfPCell(new Paragraph("Page " + pageNum + " of 5", font10));	
		}
	...
	private void generateStudentInfoTable(Document document, MBATranscriptBean bean, PdfWriter writer, Map<String, ProgramExamBean> programDetailMap) throws Exception {
		try {
			...
			addStudentInfoTableCells("Program Name / Instruction Medium: ", "MBA (Executive) with specialisation in Business Analytics / English", studentInfoTable);
//			addStudentInfoTableCells("Specialisation (Single / Dual):", bean.getSpecialisation(), studentInfoTable);
//			addStudentInfoTableCells("Duration of the Program: ", programDuration, studentInfoTable);
			addStudentInfoTableCells("Mode of Delivery: ", mod, studentInfoTable);
...
//			subjectsTable.setSpacingAfter(5f);
	...
	private void addExplainatoryDetailsPage(Document document) throws DocumentException {
		...
		addTrimesterPatternInfoTable(document);	
	}
	...
	private void addTrimesterPatternInfoTable(Document document) throws DocumentException {

		PdfPTable trimesterPatternTable = new PdfPTable(3);
		addTrimesterPatternTableHeader(trimesterPatternTable);

		addTrimesterPatternTableData("Academic Learning Activities", "1 credit", "10 hrs", trimesterPatternTable);
		addTrimesterPatternTableData("Capstone Project", "20 credits", "200 hrs (for 5 weeks)", trimesterPatternTable);
