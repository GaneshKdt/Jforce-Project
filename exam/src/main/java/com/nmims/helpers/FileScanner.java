...
* in FAR 12.212 and subject to restricted rights as defined in FAR Section 52.227-19 
* "Commercial Computer Software - Restricted Rights" and DFARS 227.7202, Rights in 
* "Commercial Computer Software or Commercial Computer Software Documentation," as applicable,
* and any successor regulations, whether delivered by Symantec as on premises or hosted services.
* Any use, modification, reproduction release, performance, display or disclosure of the 
* Licensed Software and Documentation by the U.S. Government shall be solely in accordance with 
...
/** The sample code demonstrate how to use the Symantec Protection Engine API.
 * This example demonstrates the use of both file-based scanning and
...
public class FileScanner
{
    ...
    public static void printUsage()
    {
        ...
        //System.out.println("		 Use 2 to scan file with new Insight API's, facilitates to provide file context (SHA256, MD5, digital signing status, source URL, source IP, etc) along with file scan request.Scan result provides more information about threat detected and file details.\n\n");
        ...
        else
        {
            ...
            for(int i=0; i < totalArgs; i++)
            {
                try
                {
                    ...
                    if(option.equals(scanningMode))
                    {
                        scanMode = Integer.parseInt(value);
                        argsCorrectMatch ++;
                    }
                    else
                    if(option.equals(scanStreamFileLocal))
                    {
                    ...
                    if(option.equals(scanPolicy))
                    {
                        setScanPolicy = value;
                        argsCorrectMatch ++;
                    }
                    else
                    if(option.equals(scanOutputFile))
                    {
                        scanOutput = value;
                        argsCorrectMatch ++;
                    }
                    else
                    if(option.equals(scanFileOriginalName))
                    {
                ...
                if(scanMode == 0)
                {
                    try
                    {
                        ...
                        if(isExtendedAPI == 0)
                        {
                        	...
                        	scanResult = result.getStatus().toString();
                    ...
                    if(streamFileLocal == 1)
                    {
                        try
                        {
                            ...
                            { 
	                            ...
	                            StreamScanRequest streamScanReq = scanEngine.createStreamScanRequest(fileForScan, scanFileOriginName, output, scPolicy);
                            ...
                            {
                            	ScanEngine scanEngine = ScanEngine.createScanEngine(scanEnginesForScanning);
	                            StreamScanRequest streamScanReq = scanEngine.createStreamScanRequest(fileForScan, scanFileOriginName, output, scPolicy, true);
	                            Result result = streamScanReq.scanFile();
	                            printResult(result, true);
	                            scanResult = result.getStatus().toString();
                            }
                            ...
                            {
                            	ScanEngine scanEngine = ScanEngine.createScanEngine(scanEnginesForScanning);
	                            StreamScanRequest streamScanReq = scanEngine.createStreamScanRequest(fileForScan, scanFileOriginName, output, scPolicy, disableInsightCall,insightOptions);
	                            Result result = streamScanReq.scanFile();
	                            printResult(result, true);
	                            scanResult = result.getStatus().toString();
                            }
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            if(isExtendedAPI == 0)
                            	...
                            	streamScanReq = scanEngine.createStreamScanRequest(fileForScan, scanFileOriginName, output, scPolicy, disableInsightCall, insightOptions);
