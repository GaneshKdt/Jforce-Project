...
@Component
public class ExecutiveHallTicketPDFCreator {
	...
	Font font6 = new Font(Font.FontFamily.TIMES_ROMAN, 9);
	...
	public String createHallTicket(List<ExecutiveTimetableBean> timeTableList, ArrayList<ExamBookingTransactionBean> subjectsBooked, 
			...
			HashMap<String, ExamBookingTransactionBean> subjectBookingMap, String STUDENT_PHOTOS_PATH) throws Exception{
	...
	private String generateStudentPassword(StudentExamBean student, ArrayList<ExamBookingTransactionBean> subjectsBooked) throws Exception {
			...
			/*Old logic not needed here, To be deleted after asking Sir. 15may
			 * 
			 * String maxTrackId = "";
	...
	private void generateInstructions(Document document, StudentExamBean student, String mostRecentTimetablePeriod) throws Exception {
		try {
			...
			orderedList.add(new ListItem("It is mandatory for the students to carry their own calculators (Standard/Scientific) "
						+ " as there is no on-screen calculator available. Students will not be allowed to borrow calculators "
						+ " during the examination. Non adherence will result in examination getting Null and Void.",font11));
			orderedList.add(new ListItem("Students will not be permitted to use the washroom during the examination. "
					+ " Incase of medical issue, student need to take one week prior approval from NMIMS University by sending "
					+ " medical certificate.",font11));
			...
			orderedList.add(new ListItem("For any queries do call us on 1-800-1025-136 (Mon-Sat) 9am - 7pm, during exams the operational hours are from 9am - 7pm.", font11));
	...
	private void generateStudentInfoTable(Document document, StudentExamBean student, HashMap<String, String> programCodeNameMap, 
		...
		try {
			...
			programCell.setFixedHeight(30);
			ExamCell.setFixedHeight(30);
	...
	private void generateSubjectsTable(Document document,ArrayList<ExamBookingTransactionBean> subjectsBooked,
			List<ExecutiveTimetableBean> timeTableList, HashMap<String, ExamCenterBean> examCenterIdCenterMap,
	...
	private void generateHeaderTable(Document document, StudentExamBean student, String mostRecentTimetablePeriod) throws Exception {
		...
		try {
			...
			mostRecentTimetablePeriod =  mostRecentTimetablePeriod.replaceAll("May", "May");
			mostRecentTimetablePeriod =  mostRecentTimetablePeriod.replaceAll("Jun", "June");
			mostRecentTimetablePeriod =  mostRecentTimetablePeriod.replaceAll("Jul", "July");
			mostRecentTimetablePeriod =  mostRecentTimetablePeriod.replaceAll("Aug", "August");
			mostRecentTimetablePeriod =  mostRecentTimetablePeriod.replaceAll("Sep", "September");
			mostRecentTimetablePeriod =  mostRecentTimetablePeriod.replaceAll("Oct", "October");
			mostRecentTimetablePeriod =  mostRecentTimetablePeriod.replaceAll("Nov", "November");
